...
C     ySepBnd :: Southern latitude egde (from 2 to nSepBnd, 1 is not used)
      ...
      INTEGER nSepBnd
      Real*8 ySepBnd(nSepBnd)
      Real*8 arr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,myNr,nSx,nSy)
      Real*8 arrMask(1-OLx:sNx+OLx,1-OLy:sNy+OLy,mskNr,nSx,nSy)
      Real*8 arrhFac(1-OLx:sNx+OLx,1-OLy:sNy+OLy,mskNr,nSx,nSy)
      Real*8 arrArea(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 arrY(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 arrDr(myNr)
      Real*8 theMin(nSepBnd)
      Real*8 theMax(nSepBnd)
      Real*8 theMean(nSepBnd)
      Real*8 theVar(nSepBnd)
      Real*8 theVol(nSepBnd)
      INTEGER myThid
CEOP
...
C     msgBuf :: Informational/error meesage buffer
      ...
      Real*8 tileVol (nSx,nSy,nSepDim)
      Real*8 tileMean(nSx,nSy,nSepDim)
      Real*8 tileVar (nSx,nSy,nSepDim)
     ...
     &                                 *arrMask(i,j,km,bi,bj)
      ...
      DO n=1,nSepBnd
       IF (theVol(n).GT.0.) THEN
        ...
        theMin(n) = -theMin(n)
        CALL GLOBAL_MAX_R8 ( theMin(n),  myThid )
        theMin(n)=-theMin(n)
        IF ( noPnts(n) ) theMax(n) = theMin(n)
        CALL GLOBAL_MAX_R8 ( theMax(n),  myThid )
       ENDIF
      ENDDO
