...
C            - included some CPP flags  to make
C              sure ,  are used only in
C              conjunction with defined 
...
C     ssq      :: saturation specific humidity [kg/kg] in eq. with Sea-Surface water
      ...
      Real*8 czol
      Real*8 Tsf                   ! surface temperature [K]
      Real*8 wsm                   ! limited wind speed [m/s] (> umin)
      Real*8 t0                    ! virtual temperature [K]
C     these need to be 2D-arrays for vectorizing code
      Real*8 tstar (1:sNx,1:sNy)   ! turbulent temperature scale [K]
      Real*8 qstar (1:sNx,1:sNy)   ! turbulent humidity scale  [kg/kg]
      Real*8 ustar (1:sNx,1:sNy)   ! friction velocity [m/s]
      Real*8 tau   (1:sNx,1:sNy)   ! surface stress coef = rhoA * Ws * sqrt(Cd)
      Real*8 rdn   (1:sNx,1:sNy)   ! neutral, zref (=10m) values of rd
      Real*8 rd    (1:sNx,1:sNy)   ! = sqrt(Cd)          [-]
      Real*8 delq  (1:sNx,1:sNy)   ! specific humidity difference [kg/kg]
      Real*8 deltap(1:sNx,1:sNy)

      Real*8 SSTtmp
      Real*8 ssq
      Real*8 re                    ! = Ce / sqrt(Cd)     [-]
      Real*8 rh                    ! = Ch / sqrt(Cd)     [-]
      Real*8 ren, rhn              ! neutral, zref (=10m) values of re, rh
      Real*8 usn, usm
      Real*8 stable                ! = 1 if stable ; = 0 if unstable
      Real*8 huol                  ! stability parameter at zwd [-] (=z/Monin-Obuklov length)
      Real*8 htol                  ! stability parameter at zth [-]
      Real*8 hqol
      Real*8 x                     ! stability function  [-]
      Real*8 xsq                   ! = x^2               [-]
      Real*8 psimh                 ! momentum stability function
      Real*8 psixh                 ! latent & sensib. stability function
      Real*8 zwln                  ! = log(zwd/zref)
      Real*8 ztln                  ! = log(zth/zref)
      Real*8 tmpbulk
      Real*8 recip_rhoConstFresh
      INTEGER ks, kl
      INTEGER iter
C     solve4Stress :: if F, by-pass momentum turb. flux (wind-stress) calculation
      LOGICAL solve4Stress
      Real*8 windStress            ! surface wind-stress (@ grid cell center)
...
C--   abbreviation
      recip_rhoConstFresh = 1.D0/rhoConstFresh
...
C-   Surface Temp.
           ...
           IF ( Nr.GE.2 .AND. sstExtrapol.GT.0.D0 ) THEN
            SSTtmp = sstExtrapol
     &              *( theta(i,j,ks,bi,bj)-theta(i,j,kl,bi,bj) )
     &              *  maskC(i,j,kl,bi,bj)
            Tsf = Tsf + MAX( SSTtmp, 0.D0 )
           ENDIF
...
C--  no negative evaporation over ocean:
           IF ( noNegativeEvap ) delq(i,j) = MIN( 0.D0, delq(i,j) )
...
c           evap(i,j,bi,bj) = -recip_rhonil*tau(i,j)*qstar(i,j)
     ...
     &           0.5D0 * (uVel(i,j,ks,bi,bj)+uVel(i+1,j,ks,bi,bj)) )
            vstress(i,j,bi,bj) = tmpbulk* ( vwind(i,j,bi,bj) -
     &           0.5D0 * (vVel(i,j,ks,bi,bj)+vVel(i,j+1,ks,bi,bj)) )
...
c IF ( ATEMP(i,j,bi,bj) .NE. 0. )
          ELSE
           IF ( useAtmWind ) ustress(i,j,bi,bj) = 0.D0
           IF ( useAtmWind ) vstress(i,j,bi,bj) = 0.D0
