...
C     myThid :: Instance number for this call of the routine.
      ...
      Real*8 phiHydU(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 pKappaF(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 pKappaU(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C     i,j :: Loop counters
      INTEGER i,j
      Real*8 dpk_dip, dpk_dim
      Real*8 dpk_djp, dpk_djm
...
C--   Zonal & Meridional gradient of potential anomaly
     ...
     &                  )/( dpk_dip + dpk_dim )
        dPhiHydX(i,j) = recip_dxC(i,j,bi,bj)*dPhiHydX(i,j)
c    &                * recip_deepFacC(k)*recip_rhoFacC(k)
     ...
     &                  )/( dpk_djp + dpk_djm )
        dPhiHydY(i,j) = recip_dyC(i,j,bi,bj)*dPhiHydY(i,j)
c    &                * recip_deepFacC(k)*recip_rhoFacC(k)
       ENDDO
      ENDDO
...
C--   Apply mask:
      DO j=1-OLy,sNy+OLy
       DO i=1-OLx,sNx+OLx
         dPhiHydX(i,j) = dPhiHydX(i,j)*maskW(i,j,k,bi,bj)
         dPhiHydY(i,j) = dPhiHydY(i,j)*maskS(i,j,k,bi,bj)
       ENDDO
      ENDDO
