...
C     compute divergence of stress tensor
     ...
     &       + sig12(i  ,j+1) * dxV(i,j+1,bi,bj)
     &       - sig12(i  ,j  ) * dxV(i,j  ,bi,bj)
     &       ) * recip_rAw(i,j,bi,bj)
        stressDivergenceY(i,j,bi,bj) =
     &       ( sig22(i  ,j  ) * dxF(i,j  ,bi,bj)
     &       - sig22(i  ,j-1) * dxF(i,j-1,bi,bj)
     &       + sig12(i+1,j  ) * dyU(i+1,j,bi,bj)
     &       - sig12(i  ,j  ) * dyU(i  ,j,bi,bj)
     &       ) * recip_rAs(i,j,bi,bj)
       ENDDO
      ENDDO
...
C     compute components of stress tensor from current strain rate tensor
     ...
     &       - 0.5D0 * press(i,j,bi,bj)
      ...
      DO j=1,sNy+2
       DO i=1,sNx+2
        sig12(i,j) = 2.D0 * e12(i,j,bi,bj) * etaZ(i,j,bi,bj)
       ENDDO
      ENDDO
      RETURN
      END
