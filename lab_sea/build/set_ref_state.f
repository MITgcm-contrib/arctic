...
C     pRefIntF :: reference pressure at level interface
      ...
      Real*8 rHalf(2*Nr+1), pRefIntF(Nr+1)
      Real*8 tLoc(Nr)
      Real*8 pLoc, rhoUp, rhoDw, rhoLoc
      Real*8 ddPI, conv_theta2T, thetaLoc
C--
      Real*8     maxResid
...
C-    Initialise density factor for anelastic formulation:
      DO k=1,Nr
        rhoFacC(k) = 1.D0
        recip_rhoFacC(k) = 1.D0
      ENDDO
      DO k=1,Nr+1
        rhoFacF(k) = 1.D0
        recip_rhoFacF(k) = 1.D0
      ENDDO
...
C     note: wUnit2rVel & rVel2wUnit replace horiVertRatio & recip_horiVertRatio
          ...
          rVel2wUnit(k) = 1.D0 / wUnit2rVel(k)
        ENDDO
...
C       rUnit2z = alpha/gravity : dz [m] = dr [Pa] * rUnit2z
        DO k=1,Nr
          pRef4EOS(k)  = rC(k)
          thetaLoc = tRef(k)
          IF ( thetaLoc.GT.0.D0 .AND. rC(k).GT.0.D0 ) THEN
...
C     note: wUnit2rVel & rVel2wUnit replace horiVertRatio & recip_horiVertRatio
        DO k=1,Nr+1
          ...
          ELSEIF ( k.GT.Nr ) THEN
            thetaLoc = tRef(k-1)
          ELSE
            thetaLoc = (tRef(k) + tRef(k-1))*0.5D0
          ENDIF
          IF ( thetaLoc.GT.0.D0 .AND. rF(k).GT.0.D0 ) THEN
            conv_theta2T  = (rF(k)/atm_Po)**atm_kappa
...
C-      set reciprocal rho-factor:
        DO k=1,Nr
          recip_rhoFacC(k) = 1.D0/rhoFacC(k)
        ENDDO
        DO k=1,Nr+1
          recip_rhoFacF(k) = 1.D0/rhoFacF(k)
        ENDDO
      ENDIF
