...
C     msgBuf   :: Informational/error message buffer
      INTEGER k
c     Real*8     tmpRatio, checkRatio1, checkRatio2
...
C-    first: initialise deep-model grid factor:
      DO k=1,Nr
        deepFacC(k) = 1.D0
        deepFac2C(k)= 1.D0
        recip_deepFacC(k) = 1.D0
        recip_deepFac2C(k)= 1.D0
      ENDDO
      DO k=1,Nr+1
        deepFacF(k) = 1.D0
        deepFac2F(k)= 1.D0
        recip_deepFacF(k) = 1.D0
        recip_deepFac2F(k)= 1.D0
      ENDDO
...
C-    set reciprocal of deep-model grid factor:
       DO k=1,Nr
        recip_deepFacC(k) = 1.D0/deepFacC(k)
        recip_deepFac2C(k)= 1.D0/deepFac2C(k)
       ENDDO
       DO k=1,Nr+1
        recip_deepFacF(k) = 1.D0/deepFacF(k)
        recip_deepFac2F(k)= 1.D0/deepFac2F(k)
       ENDDO
      ENDIF
