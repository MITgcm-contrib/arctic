...
C     i,j,bi,bj  :: Loop counters
      ...
      Real*8 RTmp (1:sNx,1:sNy)

      Real*8 WFAU, WFAV, WFAU2, WFAV2
      Real*8 S1, S2, S1A, S2A
      Real*8 recip_deltaT


C     diagonals of coefficient matrices
      Real*8 AU   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 BU   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 CU   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 AV   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 BV   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 CV   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     RHS of TriDiag Linear system (both directions => 5 coef A,B,C & Rt1,2)
      Real*8 rhsU (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 rhsV (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     coefficients for lateral points, u(j+/-1)
      Real*8 uRt1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 uRt2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     coefficients for lateral points, v(i+/-1)
      Real*8 vRt1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vRt2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     abbreviations
      Real*8 etaPlusZeta (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 zetaMinusEta(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     symmetric drag coefficient
      Real*8 dragSym     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     auxillary fields
      Real*8 uTmp (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vTmp (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     auxillary fields to store intermediate FORCEX/Y0
      Real*8 fxTmp(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 fyTmp(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     fractional area at velocity points
      Real*8 areaW(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 areaS(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 COSWAT
      Real*8 SINWAT
      Real*8 UERR
      LOGICAL printResidual
      Real*8 residUini, residVini, residUend, residVend
      Real*8 uRes (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vRes (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 errIni, errFD, errSum
      Real*8 residU_fd, residV_fd
      Real*8 residUmix, residVmix
      Real*8 areaMinLoc
...
C
      ...
      recip_deltaT = 1.D0 / SEAICE_deltaTdyn
...
C     initialise fractional areas at velocity points
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          areaW(i,j,bi,bj) = 1.D0
          areaS(i,j,bi,bj) = 1.D0
          ENDDO
         ENDDO
        areaMinLoc = 0.D0
...
CML         areaMinLoc = SEAICE_EPS
         DO j=jMin,jMax
          DO i=iMin,iMax
           areaW(i,j,bi,bj) =
     &          0.5D0*(AREA(i,j,bi,bj)+AREA(i-1,j,bi,bj))
           areaS(i,j,bi,bj) =
     &          0.5D0*(AREA(i,j,bi,bj)+AREA(i,j-1,bi,bj))
          ENDDO
         ENDDO
        ENDIF
       ENDDO
...
C-    add Coriolis term
     ...
     &         ( seaiceMassC(i  ,j,bi,bj) * fCori(i  ,j,bi,bj)
     &          *0.5D0*( vIceC( i ,j,bi,bj)+vIceC( i ,j+1,bi,bj) )
     &         + seaiceMassC(i-1,j,bi,bj) * fCori(i-1,j,bi,bj)
     &          *0.5D0*( vIceC(i-1,j,bi,bj)+vIceC(i-1,j+1,bi,bj) ) )
          FORCEY(i,j,bi,bj) = FORCEY(i,j,bi,bj) - HALF*
     &         ( seaiceMassC(i,j  ,bi,bj) * fCori(i,j  ,bi,bj)
     &         *0.5D0*( uIceC(i  ,j  ,bi,bj)+uIceC(i+1,  j,bi,bj) )
     &         + seaiceMassC(i,j-1,bi,bj) * fCori(i,j-1,bi,bj)
     &         *0.5D0*( uIceC(i  ,j-1,bi,bj)+uIceC(i+1,j-1,bi,bj) ) )
         ENDDO
        ENDDO
...
C-    diagonals of coefficient matrices
      ...
      Real*8 CV   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C--   seaice velocity
      Real*8 uFld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vFld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C-    residual (output)
      Real*8 residU, residV
      Real*8 uRes (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vRes (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C--   Calculate residual of the linearised system
     ...
     &                       *rAs(i,j,bi,bj)*maskInS(i,j,bi,bj)
      ...
      ENDDO
      IF ( calcMeanResid ) THEN
       CALL GLOBAL_SUM_R8 (  residU,  myThid  )
       CALL GLOBAL_SUM_R8 (  residV,  myThid  )
...
C     shear and bulk viscosity at Z points
      Real*8  etaZloc    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 zetaZloc    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     backward difference extrapolation factor (includes 1/deltaT)
      Real*8 bdfAlphaOverDt
C     strongly implicit coupling factor
      Real*8 strImpCplFac

C     fractional area at velocity points
      Real*8 areaW(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 areaS(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     coefficients of ice velocities in coefficient matrix
C     for both U and V-equation
...
C     YM: metric term with derivative in Y
      Real*8 UXX  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 UYY  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 UXM  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 UYM  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 VXX  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 VYY  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 VXM  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 VYM  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
...
C
      strImpCplFac = 0.D0
      IF ( SEAICEuseStrImpCpl ) strImpCplFac = 1.D0
      ...
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        IF ( SEAICEscaleSurfStress ) THEN
         DO j=jMin,jMax
          DO i=iMin,iMax
           areaW(i,j) = 0.5D0*(AREA(i,j,bi,bj)+AREA(i-1,j,bi,bj))
           areaS(i,j) = 0.5D0*(AREA(i,j,bi,bj)+AREA(i,j-1,bi,bj))
          ENDDO
         ENDDO
        ELSE
         DO j=jMin,jMax
...
C     ... d/dx (eta+zeta) d/dx u
          UXX(i,j) = dyF(i,j,bi,bj) * etaPlusZeta(i,j,bi,bj)
     &         * recip_dxF(i,j,bi,bj)
C     ... d/dx (zeta-eta) k1 u
          UXM(i,j) = dyF(i,j,bi,bj) * zetaMinusEta(i,j,bi,bj)
     &         * k1AtC(i,j,bi,bj) * 0.5D0
         ENDDO
        ENDDO
        DO j=jMin,jMax+1
         DO i=iMin,iMax
C     ... d/dy eta d/dy u
          UYY(i,j) = dxV(i,j,bi,bj) *
     &         ( etaZloc(i,j,bi,bj) + strImpCplFac*zetaZloc(i,j,bi,bj) )
     &         * recip_dyU(i,j,bi,bj)
C     ... d/dy eta k2 u
          UYM(i,j) = dxV(i,j,bi,bj) * etaZloc(i,j,bi,bj)
     &         * k2AtZ(i,j,bi,bj) * 0.5D0
         ENDDO
        ENDDO
        DO j=jMin,jMax
         DO i=iMin,iMax+1
C     ... d/dx eta dv/dx
          VXX(i,j) = dyU(i,j,bi,bj) *
     &         ( etaZloc(i,j,bi,bj) + strImpCplFac*zetaZloc(i,j,bi,bj) )
     &         * recip_dxV(i,j,bi,bj)
C     ... d/dx eta k1 v
          VXM(i,j) = dyU(i,j,bi,bj) * etaZloc(i,j,bi,bj)
     &         * k1AtZ(i,j,bi,bj) * 0.5D0
         ENDDO
        ENDDO
        DO j=jMin-1,jMax
         DO i=iMin,iMax
C     ... d/dy eta+zeta dv/dy
          VYY(i,j) = dxF(i,j,bi,bj) * etaPlusZeta(i,j,bi,bj)
     &         * recip_dyF(i,j,bi,bj)
C     ... d/dy (zeta-eta) k2 v
          VYM(i,j) = dxF(i,j,bi,bj) * zetaMinusEta(i,j,bi,bj)
     &         * k2AtC(i,j,bi,bj) * 0.5D0
         ENDDO
        ENDDO
...
C     than BU, hence the minus sign
     ...
     &         ( ( 1.D0 - hFacM ) * ( UYY(i  ,j  ) + UYM(i  ,j  ) )
     &         + ( 1.D0 - hFacP ) * ( UYY(i  ,j+1) - UYM(i  ,j+1) ) )
...
C     than BV, hence the minus sign
     ...
     &         ( ( 1.D0 - hFacM ) * ( VXX(i  ,j) + VXM(i  ,j) )
     &         + ( 1.D0 - hFacP ) * ( VXX(i+1,j) - VXM(i+1,j) ) )
...
C     be many cases of zero diagonal entries.
         DO j=jMin,jMax
          DO i=iMin,iMax
           IF (BU(i,j,bi,bj).EQ.0D0) BU(i,j,bi,bj) = 1.D0
           IF (BV(i,j,bi,bj).EQ.0D0) BV(i,j,bi,bj) = 1.D0
          ENDDO
         ENDDO
        ENDIF
C     bi/bj-loops
...
C
      ...
      Real*8 etaZloc     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 zetaZloc    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 pressloc    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 uIceC       (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vIceC       (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 rhsU        (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     i,j  :: Loop counters
      INTEGER i,j
      Real*8 hFacM
      Real*8 sig11(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig12(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEOP
...
C     contribution of sigma11 to rhs
     ...
     &       * 0.5D0 * ( vIceC(i,j+1,bi,bj) + vIceC(i,j,bi,bj) )
     &       - 0.5D0 * pressLoc(i,j,bi,bj)
       ENDDO
      ENDDO
C     contribution of sigma12 to rhs of u-equation
     ...
     &       * recip_dxV(i,j,bi,bj)
     &       - k1AtZ(i,j,bi,bj)
     &       * 0.5D0 * ( vIceC(i,j,bi,bj) + vIceC(i-1,j,bi,bj) )
     &       )
...
C     only need to deal with v(i)-v(i-1)
     &       + etaZloc(i,j,bi,bj) * recip_dxV(i,j,bi,bj)
     &       * ( vIceC(i,j,bi,bj) + vIceC(i-1,j,bi,bj) )
     &       * hFacM * 2.D0
       ENDDO
      ENDDO
...
C     only need to deal with u(j)-u(j-1)
     &        - zetaZloc(i,j,bi,bj) * recip_dyU(i,j,bi,bj)
     &        * ( uIceC(i,j,bi,bj) + uIceC(i,j-1,bi,bj) )
     &        * hFacM * 2.D0
        ENDDO
       ENDDO
...
C     contribution to the rhs part of grad(sigma)_x
     ...
     &       ( dyF(i  ,j  ,bi,bj)*sig11(i  ,j  )
     &       - dyF(i-1,j  ,bi,bj)*sig11(i-1,j  )
     &       + dxV(i  ,j+1,bi,bj)*sig12(i  ,j+1)
     &       - dxV(i  ,j  ,bi,bj)*sig12(i  ,j  ) )
       ENDDO
      ENDDO
...
C     i,j  :: Loop counters
      INTEGER i,j
      Real*8 hFacM
      Real*8 sig22(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig12(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEOP
...
C     contribution of sigma22 to rhs
     ...
     &       * 0.5D0 * ( uIceC(i+1,j,bi,bj) + uIceC(i,j,bi,bj) )
     &       - 0.5D0 * pressLoc(i,j,bi,bj)
       ENDDO
      ENDDO
C     contribution of sigma12 to rhs of v-equation
     ...
     &       * recip_dyU(i,j,bi,bj)
     &       - k2AtZ(i,j,bi,bj)
     &       * 0.5D0 * ( uIceC(i,j,bi,bj) + uIceC(i,j-1,bi,bj) )
     &       )
C     free slip conditions (sig12=0) are taken care of by masking sig12,
     &       *HEFFM(i  ,j  ,bi,bj)*HEFFM(i-1,j  ,bi,bj)
     &       *HEFFM(i  ,j-1,bi,bj)*HEFFM(i-1,j-1,bi,bj)
...
C     u(j)+u(j-1) = 0 is also taken care of by masking sig12, so that we
C     only need to deal with u(j)-u(j-1)
     &       + etaZloc(i,j,bi,bj) * recip_dyU(i,j,bi,bj)
     &       * ( uIceC(i,j,bi,bj) + uIceC(i,j-1,bi,bj) )
     &         * hFacM * 2.D0
       ENDDO
      ENDDO
...
C     v(i)+v(i-1) = 0 is also taken care of by masking sig12, so that we
C     only need to deal with v(i)-v(i-1)
     &        - zetaZloc(i,j,bi,bj) * recip_dxV(i,j,bi,bj)
     &        * ( vIceC(i,j,bi,bj) + vIceC(i-1,j,bi,bj) )
     &        * hFacM * 2.D0
        ENDDO
       ENDDO
...
C     contribution to the rhs part of grad(sigma)_y
     ...
     &       ( dyU(i+1,j  ,bi,bj) * sig12(i+1,j  )
     &       - dyU(i  ,j  ,bi,bj) * sig12(i  ,j  )
     &       + dxF(i  ,j  ,bi,bj) * sig22(i  ,j  )
     &       - dxF(i  ,j-1,bi,bj) * sig22(i  ,j-1) )
       ENDDO
      ENDDO
...
C     i,j  :: Loop counters
      ...
      PARAMETER ( jStep = 2 )
      Real*8 AA3, bet
      Real*8 URT(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 CUU(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C     i,j  :: Loop counters
      ...
      PARAMETER ( iStep = 2 )
      Real*8 AA3, bet
      Real*8 VRT(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 CVV(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
