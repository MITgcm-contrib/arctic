...
C     == local variables ==
      ...
      INTEGER bi,bj
      INTEGER i,j,ks
      Real*8     urelw(1:sNx,1:sNy)
      Real*8     vrelw(1:sNx,1:sNy)
      Real*8     wsLoc(1:sNx,1:sNy)
      Real*8     wsSq
      Real*8     usSq, recip_sqrtRhoA, ustar
...
C     Subtract uVel and vVel from uwind and vwind.
         DO j = 1,sNy
          DO i = 1,sNx
           urelw(i,j) = uwind(i,j,bi,bj) - 0.5D0
     &          * (uVel(i,j,ks,bi,bj)+uVel(i+1,j,ks,bi,bj))
           vrelw(i,j) = vwind(i,j,bi,bj) - 0.5D0
...
C             cdn(umps); ustar can be directly computed from stress;
          recip_sqrtRhoA = 1.D0 / SQRT(atmrho)
          DO j = 1,sNy
           DO i = 1,sNx
C     check for zero wStress to please AD tools
            IF ( wStress(i,j,bi,bj) .LE. 0.D0 ) THEN
             ustar      = 0.D0
             wsloc(i,j) = 0.D0
            ELSE
     ...
     &             (ustar*ustar/clindrag_1*0.25D0 - tmp1*tmp1*tmp1 )
     &                    )
