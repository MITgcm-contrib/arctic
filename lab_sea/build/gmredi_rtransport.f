...
C-    Horizontal gradients interpolated to W points
     ...
     &       *( maskW(i+1,j,k,bi,bj)*recip_dxC(i+1,j,bi,bj)
     &             *( Tracer(i+1,j,k) - Tracer( i ,j,k) )
     &        + maskW( i ,j,k,bi,bj) *recip_dxC(i,j,bi,bj)
     &             *( Tracer( i ,j,k) - Tracer(i-1,j,k) )
     &        )
     &   +op5*recip_deepFacC(k-1)
     &       *( maskW(i+1,j,k-1,bi,bj)*recip_dxC(i+1,j,bi,bj)
     ...
     &        )          )
        ...
        dTdy(i,j) = op5*(
     &    op5*recip_deepFacC(k)
     &       *( maskS(i,j+1,k,bi,bj)*recip_dyC(i,j+1,bi,bj)
     &             *( Tracer(i,j+1,k) - Tracer(i,j,k) )
     &        + maskS(i, j ,k,bi,bj)*recip_dyC(i, j ,bi,bj)
     &             *( Tracer(i,j,k) - Tracer(i,j-1,k) )
     &        )
     &   +op5*recip_deepFacC(k-1)
     &       *( maskS(i,j+1,k-1,bi,bj)*recip_dyC(i,j+1,bi,bj)
     &             *( Tracer(i,j+1,k-1) - Tracer(i,j,k-1) )
     &        + maskS(i, j ,k-1,bi,bj)*recip_dyC(i, j ,bi,bj)
...
C-    Off-diagonal components of vertical flux
      IF ( trIdentity .GE. GAD_TR1 ) THEN
        DO j=jMin,jMax
         DO i=iMin,iMax
          df(i,j) = df(i,j)
     &      - rA(i,j,bi,bj)*deepFac2F(k)*maskInC(i,j,bi,bj)
