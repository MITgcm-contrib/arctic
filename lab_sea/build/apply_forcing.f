...
C     myThid    :: my Thread Id number
      Real*8     gU_arr(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C     myIter    :: Current iteration number
C     myThid    :: my Thread Id number
      Real*8     gV_arr(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      INTEGER iMin, iMax, jMin, jMax
      INTEGER k, bi, bj
      Real*8     myTime
      INTEGER myIter
...
C     myIter    :: Current iteration number
C     myThid    :: my Thread Id number
      Real*8     gT_arr(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      INTEGER iMin, iMax, jMin, jMax
      INTEGER k, bi, bj
      Real*8     myTime
      INTEGER myIter
       ...
       DO j=0,sNy+1
        DO i=0,sNx+1
         IF (k .EQ. kSurfC(i,j,bi,bj)) THEN
          gT_arr(i,j) = gT_arr(i,j)
     &        +TsurfCor*recip_drF(k)*recip_hFacC(i,j,k,bi,bj)
         ENDIF
        ENDDO
       ENDDO
      ENDIF
...
C     Put depth of interface above & below current level in swfracb (1 & 2)
...
C     myIter    :: Current iteration number
C     myThid    :: my Thread Id number
      Real*8     gS_arr(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      INTEGER iMin, iMax, jMin, jMax
      INTEGER k, bi, bj
      Real*8     myTime
      INTEGER myIter
      ...
      IF (linFSConserveTr) THEN
       DO j=0,sNy+1
        DO i=0,sNx+1
         IF (k .EQ. kSurfC(i,j,bi,bj)) THEN
          gS_arr(i,j) = gS_arr(i,j)
     &        +SsurfCor*recip_drF(k)*recip_hFacC(i,j,k,bi,bj)
         ENDIF
        ENDDO
       ENDDO
      ENDIF
