...
C     myThid :: my Thread Id Number
      Real*8 X(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 Y(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      INTEGER myThid
CEOP
...
C     Euler angles in radians
      Real*8 phiRad, thetaRad, psiRad
C     inverted rotation matrix
      Real*8 Ainv(3,3), Binv(3,3), Cinv(3,3), Dinv(3,3), CB(3,3)
C     cartesian coordinates
      Real*8 XYZgeo(3), XYZrot(3)
...
C     tranform cartesian coordinates back to spherical coordinates
     ...
     &                  + ABS(XYZgeo(2))*ABS(XYZgeo(2)) )
          IF(XYZgeo(1) .EQ. 0.D0 .AND. XYZgeo(2) .EQ. 0.D0)THEN
C     happens exactly at the poles
           X(I,J,bi,bj) = 0.D0
          ELSE
           X(I,J,bi,bj) = ATAN2(XYZgeo(2),XYZgeo(1))/deg2rad
           IF ( X(I,J,bi,bj) .LT. 0.D0 )
     &          X(I,J,bi,bj) = X(I,J,bi,bj) + 360.D0
          ENDIF
          IF(hypotxy .EQ. 0.D0 .AND. XYZgeo(3) .EQ. 0.D0)THEN
