...
C     !LOCAL VARIABLES:
      INTEGER bi,bj,i,j,k
      INTEGER iMax,jMax
      Real*8 theVol, theArea, tmpVal, tmpAre, tmpVol
      Real*8 theMin, theMax, theMean, theVar, volMean, volVar, theSD
c     Real*8 areaTile, volTile, sumTile, sqsTile, vSumTile, vSqsTile
      Real*8 tileArea(nSx,nSy)
      Real*8 tileVol (nSx,nSy)
      Real*8 tileSum (nSx,nSy)
      Real*8 tileVar (nSx,nSy)
      Real*8 tileVSum(nSx,nSy)
      Real*8 tileVSq (nSx,nSy)
...
C-    Special stuff for North & South Poles, LatLon grid
     ...
     &                 + uVel(i,j-1,k,bi,bj)*dxC(i,j-1,bi,bj)
            hFacZ(i,j) = 0.
            hFacZ(1,j) = hFacZ(1,j) + hFacW(i,j-1,k,bi,bj)
           ENDDO
     ...
     &                 - uVel(i,j,k,bi,bj)*dxC(i,j,bi,bj)
            hFacZ(i,j) = 0.
            hFacZ(1,j) = hFacZ(1,j) + hFacW(i,j,k,bi,bj)
           ENDDO
            hFacZ(1,j) = hFacZ(1,j) / FLOAT(sNx)
            vort3(1,j) = vort3(1,j)*recip_rAz(1,j,bi,bj)
          ENDIF
...
c       volVar = volVar  + tileVSq(bi,bj)
      ...
      theMin = -theMin
c     CALL GLOBAL_SUM_R8 ( theArea, myThid )
c     CALL GLOBAL_SUM_R8 ( theVol,  myThid )
c     CALL GLOBAL_SUM_R8 ( theMean, myThid )
c     CALL GLOBAL_SUM_R8 ( theVar,  myThid )
c     CALL GLOBAL_SUM_R8 ( volMean, myThid )
c     CALL GLOBAL_SUM_R8 ( volVar , myThid )
