...
C
      ...
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        ...
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          d_HEFFbyNEG(i,j,bi,bj)    = 0.0D0
          d_HSNWbyNEG(i,j,bi,bj)    = 0.0D0
         ENDDO
        ENDDO
...
C--   (1) treat the case of negative values:
        ...
        DO j=1,sNy
         DO i=1,sNx
          d_HEFFbyNEG(i,j,bi,bj)=MAX(-HEFF(i,j,bi,bj),0.D0)
          HEFF(i,j,bi,bj)=HEFF(i,j,bi,bj)+d_HEFFbyNEG(i,j,bi,bj)
          d_HSNWbyNEG(i,j,bi,bj)=MAX(-HSNOW(i,j,bi,bj),0.D0)
          HSNOW(i,j,bi,bj)=HSNOW(i,j,bi,bj)+d_HSNWbyNEG(i,j,bi,bj)
          AREA(i,j,bi,bj)=MAX(AREA(i,j,bi,bj),0.D0)
         ENDDO
        ENDDO
...
C--   (3) treat the case of area but no ice/snow:
         ...
         DO j=1,sNy
          DO i=1,sNx
           IF ((HEFF(i,j,bi,bj).EQ.0.D0).AND.
     &        (HSNOW(i,j,bi,bj).EQ.0.D0)) AREA(i,j,bi,bj)=0.D0
         ENDDO
        ENDDO
