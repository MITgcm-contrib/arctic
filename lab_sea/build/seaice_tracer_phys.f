...
C     === Local variables ===
      ...
      Real*8 SItrFromOcean  (1:sNx,1:sNy)
      Real*8 SItrFromFlood   (1:sNx,1:sNy)
      Real*8 HEFFprev, HEFFpost, growFact, meltPart, tmpscal1
      Real*8 SItrExpand  (1:sNx,1:sNy)
      Real*8 AREAprev, AREApost, expandFact
...
c update SItr accordingly
     ...
     &                      +SItrFromOcean(i,j)*(1.D0 - growFact)
         SItrBucket(i,j,bi,bj,iTr)=SItrBucket(i,j,bi,bj,iTr)
     &             -HEFFpost*SItrFromOcean(i,j)*(1.D0 - growFact)
         SItrBucket(i,j,bi,bj,iTr)=SItrBucket(i,j,bi,bj,iTr)
...
c apply flooding term
        ...
        if (HEFFpost.GT.HEFFprev) growFact=HEFFprev/HEFFpost
        SItracer(i,j,bi,bj,iTr)=SItracer(i,j,bi,bj,iTr)*growFact
     &                     +SItrFromFlood(i,j) *(1.D0 - growFact)
...
c as we dont have snow tracers, so it goes through SItrBucket.
          SItrBucket(i,j,bi,bj,iTr)=SItrBucket(i,j,bi,bj,iTr)
     &             -HEFFpost*SItrFromFlood(i,j)*(1.D0 - growFact)
       ENDDO
      ENDDO
...
c update SItr accordingly
     ...
     &                      +SItrExpand(i,j)*(1.D0 - expandFact)
       ENDDO
      ENDDO
      endif
...
c age tracer: grow old as time passes by
       DO J=1,sNy
        DO I=1,sNx
          if (( (SItrHEFF(i,j,bi,bj,5).GT.0.D0).AND.(SItrMate(iTr)
     &     .EQ.'HEFF') ).OR.( (SItrAREA(i,j,bi,bj,3).GT.0.D0).AND.
...
c ridging increases roughness
     ...
     &    MAX(0.D0, SItrAREA(i,j,bi,bj,1)-SItrAREA(i,j,bi,bj,2))
