...
CBOP
C !ROUTINE: MDS_PASS_R8toRL
C !INTERFACE:
      SUBROUTINE MDS_PASS_R8toRL(
     U                            buffer, arrFld,
...
C myThid  (integer):: my Thread Id number
      ...
      Real*8    arrFld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,kSize,nSx,nSy)
      INTEGER kLo
      INTEGER biArg
      INTEGER bjArg
      LOGICAL copyTo
...
CEOP
     ...
     &     oLj.LT.0 .OR. oLj.GT.OLy ) THEN
        STOP 'ABNORMAL END: MDS_PASS_R8toRL invalid oLi,oLj Arg'
      ENDIF
     ...
     &   .AND. bjArg.GE.1 .AND. bjArg.LE.nSy ) THEN
        ...
        ENDIF
      ELSE
        STOP 'ABNORMAL END: MDS_PASS_R8toRL invalid bi,bj Arg'
      ENDIF
