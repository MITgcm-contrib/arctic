...
C     betaFacU/V     :: analog betaFacP1U/V
      ...
      Real*8 COSWAT
      Real*8 SINWAT
      Real*8 ecc2, recip_ecc2, recip_evpAlpha, recip_deltaT
      Real*8 betaFacP1, betaFac, evpStarFac, evpRevFac, recip_evpRevFac

      Real*8 seaice_div    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 seaice_tension(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 seaice_shear  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig11         (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig22         (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     fractional area at velocity points
      Real*8 areaW         (1:sNx,1:sNy,nSx,nSy)
      Real*8 areaS         (1:sNx,1:sNy,nSx,nSy)
C     auxilliary variables
      Real*8 ep            (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 em            (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 e12Csq        (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 pressC        (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 zetaC         (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 deltaZ        (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     Real*8 zetaZ         (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 deltaCreg, deltaSq, deltaMinSq, tmp
      Real*8 denom1   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 denom2   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 sumNorm, denomU, denomV
      Real*8 locMaskU, locMaskV
      Real*8 EVPcFac
      Real*8 evpAlphaC(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 evpAlphaZ(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 evpBetaU (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 evpBetaV (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 betaFacP1U,  betaFacP1V
      Real*8 betaFacU,    betaFacV
...
C     abbreviation eccentricity squared
      ecc2=SEAICE_eccen**2
      recip_ecc2 = 0.D0
      IF ( ecc2 .NE. 0.D0 ) recip_ecc2=ONE/ecc2
...
C     SEAICE_evpAlpha = 2. * SEAICE_evpTauRelax/SEAICE_deltaTevp
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          denom1(i,j,bi,bj) = 1.D0 / ( SEAICE_evpAlpha + 1.D0 )
          denom2(i,j,bi,bj) = 1.D0 / ( SEAICE_evpAlpha + ecc2 )
         ENDDO
        ENDDO
       ENDDO
      ENDDO
      ...
      recip_evpAlpha = 0.D0
      IF ( SEAICE_evpAlpha .GT. 0.D0 )
     &     recip_evpAlpha = 1.D0 / SEAICE_evpAlpha
      evpStarFac = 0.D0
      evpRevFac  = 0.D0
      recip_evpRevFac = 1.D0
      IF ( SEAICEuseEVPstar ) evpStarFac = 1.D0
...
C     explicit terms
       ...
       DO bj=myByLo(myThid),myByHi(myThid)
        DO bi=myBxLo(myThid),myBxHi(myThid)
         DO j=1-OLy,sNy+OLy
          DO i=1-OLx,sNx+OLx
           denom1(i,j,bi,bj)     = 1.D0 / SEAICE_evpAlpha
        ...
        DO bj=myByLo(myThid),myByHi(myThid)
         DO bi=myBxLo(myThid),myBxHi(myThid)
          DO j=1-OLy,sNy+OLy
           DO i=1-OLx,sNx+OLx
            seaice_div    (i,j) = 0.D0
            seaice_tension(i,j) = 0.D0
            seaice_shear  (i,j) = 0.D0
            pressC        (i,j) = 0.D0
            e12Csq        (i,j) = 0.D0
            zetaC         (i,j) = 0.D0
            deltaZ        (i,j) = 0.D0
            zetaZ   (i,j,bi,bj) = 0.D0
            deltaC  (i,j,bi,bj) = 0.D0
           ENDDO
          ENDDO
          DO j=1-OLy,sNy+OLy
           DO i=1-OLx,sNx+OLx
...
C     (and energy conserving) according to Bouillon et al. 2013, eq (11)
             e12Csq(i,j) = 0.25D0 * recip_rA(i,j,bi,bj) *
...
C     recompute pressure
          DO j=0,sNy+1
           DO i=0,sNx+1
            pressC(i,j) =
     &           ( press0(i,j,bi,bj) * ( 1.D0 - SEAICEpressReplFac )
...
C     P * ( D_s/Delta     ) = 2*zeta*D_s
          IF (.TRUE. ) THEN
           DO j=0,sNy
            DO i=0,sNx
             seaice_div    (i,j) =
     &            ( 2.D0 *zetaC(i,j)*ep(i,j) - pressC(i,j)
     &            ) * HEFFM(i,j,bi,bj)
             seaice_tension(i,j) = 2.D0*zetaC(i,j)
     &            * em(i,j) * HEFFM(i,j,bi,bj)
           ...
           DO j=1,sNy+1
            DO i=1,sNx+1
             seaice_shear  (i,j) =
     &            2.D0*zetaZ(i,j,bi,bj)*e12(i,j,bi,bj)
            ENDDO
           ENDDO
          ENDIF
C
...
C
          IF ( useAdaptiveEVP ) THEN
           DO j=0,sNy
            DO i=0,sNx
             denom1(i,j,bi,bj) = 1.D0 /  evpAlphaC(i,j,bi,bj)
             denom2(i,j,bi,bj) = denom1(i,j,bi,bj)
            ENDDO
           ENDDO
          ENDIF
...
C
     ...
     &           ( sig11(i  ,j  ) * dyF(i  ,j,bi,bj)
     &           - sig11(i-1,j  ) * dyF(i-1,j,bi,bj)
     &           + seaice_sigma12(i,j+1,bi,bj) * dxV(i,j+1,bi,bj)
     &           - seaice_sigma12(i,j  ,bi,bj) * dxV(i,j  ,bi,bj)
     &           ) * recip_rAw(i,j,bi,bj)
            stressDivergenceY(i,j,bi,bj) =
     &           ( sig22(i,j  ) * dxF(i,j  ,bi,bj)
     &           - sig22(i,j-1) * dxF(i,j-1,bi,bj)
     &           + seaice_sigma12(i+1,j,bi,bj) * dyU(i+1,j,bi,bj)
     &           - seaice_sigma12(i  ,j,bi,bj) * dyU(i  ,j,bi,bj)
     &           ) * recip_rAs(i,j,bi,bj)
           ENDDO
          ENDDO
         ENDDO
...
C     propagate into the ice covered regions.
            ...
            IF ( locMaskU .NE. 0.D0 ) locMaskU = 1.D0
            IF ( locMaskV .NE. 0.D0 ) locMaskV = 1.D0
...
C     compute and adjust parameters that are constant otherwise
             evpBetaU(i,j,bi,bj) = 0.5D0*(evpAlphaC(i-1,j,bi,bj)
     &                                      +evpAlphaC(i,  j,bi,bj))
             evpBetaV(i,j,bi,bj) = 0.5D0*(evpAlphaC(i,j-1,bi,bj)
     &                                      +evpAlphaC(i,j,  bi,bj))
     ...
     &           * 0.5D0*( CbotC(i,j,bi,bj) + CbotC(i-1,j,bi,bj) )
            denomV = denomV + areaS(i,j,bi,bj)
     &           * 0.5D0*( CbotC(i,j,bi,bj) + CbotC(i,j-1,bi,bj) )
            IF ( denomU .EQ. 0.D0 ) denomU = 1.D0
            IF ( denomV .EQ. 0.D0 ) denomV = 1.D0
