...
C     myThid     :: my Thread Id. number
      Real*8 UI     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 VI     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
CML   lat-lon-grid, but may not be accurate on irregular grids
     ...
     &           ( tmpFld(i  ,j  ,bi,bj)+tmpFld(i  ,j-1,bi,bj))
     &           *   VI(i  ,j  ,  bi,bj)
     &           * dxG(i,j,bi,bj))*maskInC(i,j,bi,bj)
     &           *(HALF * recip_dyF(i,j,bi,bj) * recip_dxF(i,j,bi,bj))
           ENDDO
          ENDDO
         ENDDO
        ENDDO
...
C--   first compute fluxes across cell faces
          DO j=1,sNy+1
           DO i=1,sNx+1
            afx(i,j) = dyG(i,j,bi,bj) * UI(i,j,bi,bj)
     &           * 0.5D0 * (tmpFld(i,j,bi,bj)+tmpFld(i-1,j,bi,bj))
            afy(i,j) = dxG(i,j,bi,bj) * VI(i,j,bi,bj)
     &           * 0.5D0 * (tmpFld(i,j,bi,bj)+tmpFld(i,j-1,bi,bj))
           ENDDO
          ENDDO
          DO j=1,sNy
           DO i=1,sNx
...
C  1rst pass: compute changes due to harmonic diffusion and add it to ice-field
       DO bj=myByLo(myThid),myByHi(myThid)
        DO bi=myBxLo(myThid),myBxHi(myThid)
          DO j=1-OLy,sNy+OLy
           DO i=1-OLx,sNx+OLx
            DIFFA(i,j,bi,bj) = MIN( dxF(i,j,bi,bj), dyF(i,j,bi,bj) )
           ENDDO
          ENDDO
        ENDDO
       ENDDO
