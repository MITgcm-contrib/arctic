...
C     Calculate bare statistics of global array ``\Real*8 arr''.
...
C     !INPUT PARAMETERS:
      ...
      Real*8 arrMask(1-OLx:sNx+OLx,1-OLy:sNy+OLy,myNr,nSx,nSy)
      Real*8 arrhFac(1-OLx:sNx+OLx,1-OLy:sNy+OLy,myNr,nSx,nSy)
      Real*8 arrArea(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 arrDr(myNr)
      Real*8 theMin, theMax, theMean, theSD, theDel2, theVol
      INTEGER myThid
CEOP
...
C     !LOCAL VARIABLES:
      ...
      Real*8 theVar
      Real*8 tmpVol
      Real*8 tileMean(nSx,nSy)
      Real*8 tileVar (nSx,nSy)
      Real*8 tileSD  (nSx,nSy)
      Real*8 tileDel2(nSx,nSy)
...
c     CALL GLOBAL_SUM_R8 ( theDel2, myThid )
...
c      CALL GLOBAL_SUM_R8 ( theSD, myThid )
