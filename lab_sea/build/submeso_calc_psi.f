...
C     == Local variables ==
      INTEGER i,j,k
      Real*8 mixLayerU   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 mixLayerV   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 dBuoyX_Hu   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 dBuoyY_Hv   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 NHmixLay    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 MsquareH    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 lengthScaleF(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 fcorLoc     (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 PsiLoc      (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 dzLoc
      Real*8 five_ov21
      PARAMETER( five_ov21 = 5.D0 / 21.D0 )

C--   parameter to move to GMREDI.h
c     Real*8 subMeso_invTau, subMeso_LfMin, subMeso_Ceff
c     Real*8 subMeso_Lmax

c     subMeso_invTau = 1.6D-6  ! ~ 1/(7.2 days)
c     subMeso_LfMin  = 1000.D0
c     subMeso_Ceff   = 0.07D0
c     subMeso_Lmax   = 111.D3
...
C--    Integrate buoyancy gradient over the Mixed-Layer
     ...
     &                       + subMeso_invTau*subMeso_invTau )
       ...
       DO k=1,Nr
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx+1,sNx+OLx
          dzLoc = MAX( 0.D0, MIN( drF(k), mixLayerU(i,j)+rF(k) ) )
          dBuoyX_Hu(i,j) = dBuoyX_Hu(i,j) + sigmaX(i,j,k)*dzLoc
         ENDDO
        ENDDO
        DO j=1-OLy+1,sNy+OLy
         DO i=1-OLx,sNx+OLx
          dzLoc = MAX( 0.D0, MIN( drF(k), mixLayerV(i,j)+rF(k) ) )
...
C      Mix-Layer Eddies contribution to Bolus Transport in X dir.
     ...
     &                 *2.D0/(lengthScaleF(i-1,j)+lengthScaleF(i,j))
     &                 *2.D0/(fcorLoc(i-1,j)+fcorLoc(i,j))
        ENDDO
       ENDDO
...
C      Mix-Layer Eddies contribution to Bolus Transport in Y dir.
     ...
     &                 *2.D0/(lengthScaleF(i,j-1)+lengthScaleF(i,j))
     &                 *2.D0/(fcorLoc(i,j-1)+fcorLoc(i,j))
        ENDDO
       ENDDO
       IF ( useDiagnostics ) THEN
