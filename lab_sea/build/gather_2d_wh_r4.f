...
CBOP
C !ROUTINE: GATHER_2D_WH_R4
C !INTERFACE:
      SUBROUTINE GATHER_2D_WH_R4(
     O                  gloBuff,
     I                  procBuff,
     I                  myThid )
C !DESCRIPTION:
...
C gloBuff   ( Real*4 ) :: full-domain 2D IO-buffer array             (Output)
C procBuff  ( Real*4 ) :: proc-domain 2D IO-buffer array             (Input)
...
C     gloNyWh :: global sum of sNyWh
      ...
      Real*4     gloBuff(sNxWh,gloNyWh)
      Real*4     procBuff(sNxWh,procNyWh)
      INTEGER myThid
CEOP
