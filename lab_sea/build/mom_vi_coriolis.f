...
C   epsil          :: small number
      INTEGER i,j
      Real*8 uBarXY, vBarXY
      Real*8 uBarYm, uBarYp, vBarXm, vBarXp
      Real*8 epsil
CEOP
...
C- hFac weighted average :
     ...
     &               )
         uCoriolisTerm(i,j)=
     &    +0.5*( fCoriG(i,j,bi,bj)+fCoriG(i,j+1,bi,bj)
     &         )*vBarXY*recip_dxC(i,j,bi,bj)*recip_hFacW(i,j,k,bi,bj)
        ENDDO
       ENDDO
...
C- Energy-conserving discretisation with hFac weighted average :
     ...
     &       vFld( i ,j+1)*dxG( i ,j+1,bi,bj)*hFacS( i ,j+1,k,bi,bj)
     &      +vFld(i-1,j+1)*dxG(i-1,j+1,bi,bj)*hFacS(i-1,j+1,k,bi,bj) )
         uCoriolisTerm(i,j) = +0.5D0
     &        *( vBarXm*fCoriG(i, j ,bi,bj)
     &          +vBarXp*fCoriG(i,j+1,bi,bj)
     &         )*recip_dxC(i,j,bi,bj)*recip_hFacW(i,j,k,bi,bj)
        ENDDO
       ENDDO
      ELSE
...
C- hFac weighted average :
     ...
     &               )
         vCoriolisTerm(i,j)=
     &    -0.5*( fCoriG(i,j,bi,bj)+fCoriG(i+1,j,bi,bj)
     &         )*uBarXY*recip_dyC(i,j,bi,bj)*recip_hFacS(i,j,k,bi,bj)
        ENDDO
       ENDDO
      ELSEIF ( selectCoriScheme .EQ. 3 ) THEN
C- Energy-conserving discretisation with hFac weighted average :
     ...
     &       uFld(i+1, j )*dyG(i+1, j ,bi,bj)*hFacW(i+1, j ,k,bi,bj)
     &      +uFld(i+1,j-1)*dyG(i+1,j-1,bi,bj)*hFacW(i+1,j-1,k,bi,bj) )
         vCoriolisTerm(i,j) = -0.5D0
     &        *( uBarYm*fCoriG( i ,j,bi,bj)
     &          +uBarYp*fCoriG(i+1,j,bi,bj)
     &         )*recip_dyC(i,j,bi,bj)*recip_hFacS(i,j,k,bi,bj)
        ENDDO
       ENDDO
      ELSE
        STOP 'MOM_VI_CORIOLIS: invalid selectCoriScheme'
