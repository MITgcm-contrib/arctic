...
C     myThid             :: my thread identification number
      Real*8     fldStartTime, fldPeriod, fldRepeatCycle
      CHARACTER*(*) fldName
...
C     myDateSeconds :: seconds from beginning of year to mydate
      ...
      INTEGER difftime(4)
      Real*8 fldsectot, fldsecs, fldsecs0, fldsecs1
      Real*8 secondsInYear, myDateSeconds
       INTEGER intimeP, intime0, intime1
       Real*8 locTime, aWght, bWght
...
C     Determine the flux records just before and after mycurrentdate.
         if ( fldRepeatCycle .eq. 0.D0 ) then

          if ( fldsectot .lt. 0.D0 ) then
...
C     Set switch for reading new record.
       if ( fldsecs - modelstep .lt. 0.D0 ) changed = .TRUE.
