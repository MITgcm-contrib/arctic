...
C     (point i,j belong to region "n" <=> regMask(i,j,kRegMsk(n)) = vRegMsk(n))
      ...
      Real*8       diagSt_regMask(1-Olx:1-Olx,1-Oly:1-Oly,1,nSx,nSy)
c     Real*8       diagSt_regMask(1-OLx:sNx+Olx,1-Oly:sNy+Oly,1,nSx,nSy)
      Real*8       diagSt_vRegMsk(0:nRegions)
...
C     stat_fName  :: List of statistics output Filename
      ...
      Real*8         frequency(ldimLoc), timePhase(ldimLoc)
      Real*8         averagingFreq(ldimLoc), averagingPhase(ldimLoc)
      INTEGER     repeatCycle(ldimLoc)
      Real*8         missing_value(ldimLoc)
      INTEGER     missing_value_int(ldimLoc)
      Real*8         levels(kdimLoc,ldimLoc)
      Real*8         stat_freq(ldimLoc), stat_phase(ldimLoc)
...
c       missing_value(l)     = UNSET_RL
     ...
     &  diagCG_pcOffDFac = 0.25D0 /( cg2dpcOffDFac*cg2dpcOffDFac )
      xPsi0 = UNSET_RS
      yPsi0 = UNSET_RS
...
C-     Only lists with non-empty file name (iLen>0) are considered
       IF ( iLen.GE.1 .AND. diagSt_nbLists.LT.numLists)THEN
         ...
         ELSEIF ( stat_freq(l) .LT. 0. ) THEN
           diagSt_phase(n) = -0.5D0 * stat_freq(l)
         ENDIF
