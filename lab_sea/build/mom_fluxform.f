...
C           +\frac{1}{\rho} \nabla \cdot {\bf \tau}^x
C           + \mbox{metrics}
C \C G^v & = & -\frac{1}{\rho} \partial_y \phi_h
C           -\nabla \cdot {\bf v} v
C           +fu
C           +\frac{1}{\rho} \nabla \cdot {\bf \tau}^y
C           + \mbox{metrics}
...
C- to switch back to (very) old accounting of geometry/Area factor
...
C  myThid               :: my Thread Id number
      ...
      Real*8 kappaRU(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr+1)
      Real*8 kappaRV(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr+1)
      Real*8 fVerUkm(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 fVerVkm(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 fVerUkp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 fVerVkp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C     uDudxFac, AhDudxFac, etc ... individual term parameters for switching terms off
      ...
      Real*8   h0FacZ(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      ...
      Real*8 stretching(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  uDudxFac
      Real*8  AhDudxFac
      Real*8  vDudyFac
      Real*8  AhDudyFac
