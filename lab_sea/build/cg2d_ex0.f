...
C     cg2d_s     ::   *same*
      ...
      Real*8    cg2dTolerance_sq
      Real*8    err_sq,  errTile(nSx,nSy)
      Real*8    eta_qrNtile(nSx,nSy)
      Real*8    eta_qrNM1(nSx,nSy)
      Real*8    cgBeta
      Real*8    alpha,   alphaTile(nSx,nSy)
      Real*8    sumRHS,  sumRHStile(nSx,nSy)
      Real*8    rhsMax, rhsMaxLoc
      Real*8    rhsNorm(nSx,nSy)
      Real*8    minResTile(nSx,nSy)
      Real*8    cg2d_min(1:sNx,1:sNy,nSx,nSy)
      Real*8    cg2d_q  (1:sNx,1:sNy,nSx,nSy)
...
C--   Initialise inverter and Normalise RHS
      ...
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        ...
        actualIts(bi,bj)  = 0
        minResIter(bi,bj) = 0
        minResTile(bi,bj) = -1.D0
        eta_qrNM1(bi,bj) =  1.D0
        rhsMaxLoc = 0.D0
        DO j=1,sNy
         DO i=1,sNx
        ...
        ENDDO
        rhsNorm(bi,bj) = 1.D0
        IF ( rhsMaxLoc .NE. 0. ) rhsNorm(bi,bj) = 1.D0 / rhsMaxLoc
