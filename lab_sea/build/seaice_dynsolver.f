...
C     bi,bj  :: tile counters
      INTEGER i, j, bi, bj
      Real*8 mask_uice, mask_vice
C     phiSurf :: geopotential height at sea surface (including pressure load)
      Real*8 phiSurf(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C     TAUY    :: meridional wind stress over seaice at V point
      ...
      Real*8 sig1   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig2   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig12  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEAW[ Define sigma variables      
      Real*8 sigma11   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sigma22   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sigma12   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEAW]
      Real*8 sigp, sigm, sigTmp, recip_prs
      Real*8 areaW, areaS, COSWAT
      Real*8 SINWAT
CEAW[ Define sea surface tilt and seaice velocities (used at same timestep)
      Real*8 TILTx (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 TILTy (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 SIuvel (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 SIvvel (1-OLx:sNx+OLx,1-OLy:sNy+OLy) 
CEAW]
      INTEGER kSrf
...
CEAW[ Create logical for when I want to output sigma tensor
      LOGICAL diag_SIsig_isOn
CEAW] 
chzh[
      LOGICAL diag_SIstr_isOn
chzh]
CEOP
         ...
         DO bi=myBxLo(myThid),myBxHi(myThid)
          DO j=1-OLy+1,sNy+OLy
           DO i=1-OLx+1,sNx+OLx
            ...
            IF ( (seaiceMaskU(i,j,bi,bj) .GT. 0.D0) .AND.
     &           (mask_uice .GT. 1.5D0) ) THEN
             seaiceMaskU(i,j,bi,bj) = 1.D0
            ELSE
             seaiceMaskU(i,j,bi,bj) = 0.D0
            ENDIF
            seaiceMaskV(i,j,bi,bj)=AREA(i,j,bi,bj)+AREA(i,j-1,bi,bj)
            mask_vice=HEFFM(i,j,bi,bj)+HEFFM(i  ,j-1,bi,bj)
            IF ( (seaiceMaskV(i,j,bi,bj) .GT. 0.D0) .AND.
     &           (mask_vice .GT. 1.5D0) ) THEN
             seaiceMaskV(i,j,bi,bj) = 1.D0
            ELSE
             seaiceMaskV(i,j,bi,bj) = 0.D0
            ENDIF
           ENDDO
          ENDDO
         ENDDO
...
CEAW[ Based on the above block, save FORCE regardless of which if statement
      IF ( useDiagnostics ) THEN
       CALL DIAGNOSTICS_FILL(FORCEX0, 'FORCEX0 ',0,1,0,1,1,myThid)
       CALL DIAGNOSTICS_FILL(FORCEY0, 'FORCEY0 ',0,1,0,1,1,myThid)
      ENDIF
CEAW]
         ...
         IF ( SEAICEuseTILT ) THEN
          DO j=1-OLy+1,sNy+OLy
           DO i=1-OLx+1,sNx+OLx
C--   now add in tilt
CEAW[ Separated the tilt portion from FORCE so I can output it
	    TILTx(i,j,bi,bj) =  
     &           -seaiceMassU(i,j,bi,bj)*recip_dxC(i,j,bi,bj)
     &           *( phiSurf(i,j)-phiSurf(i-1,j) )
	    TILTy(i,j,bi,bj) =  
     &           -seaiceMassV(i,j,bi,bj)* recip_dyC(i,j,bi,bj)
     &           *( phiSurf(i,j)-phiSurf(i,j-1) )
            FORCEX0(i,j,bi,bj)=FORCEX0(i,j,bi,bj) + TILTx(i,j,bi,bj)
            FORCEY0(i,j,bi,bj)=FORCEY0(i,j,bi,bj) + TILTy(i,j,bi,bj)
CEAW]
           ENDDO
          ENDDO
CEAW[ Fill the tilt diagnsotc
      IF ( useDiagnostics ) THEN
         ...
         DO bi=myBxLo(myThid),myBxHi(myThid)
          DO j=1-OLy,sNy+OLy
           DO i=1-OLx,sNx+OLx
            ...
            stressDivergenceX(i,j,bi,bj) = 0.D0
            stressDivergenceY(i,j,bi,bj) = 0.D0
           ENDDO
          ENDDO
         ENDDO
        ENDDO
...
C     but we are looking for T = k*P = k*press/(1-k)
     ...
     &            *press(i,j,bi,bj)/(1.D0 - tensileStrFac(i,j,bi,bj))
            ENDIF
           ENDDO
          ENDDO
...
CEAW[ Add divsig and SIuvel to this flag based on structure of calcs       
     &                 .OR. DIAGNOSTICS_IS_ON('divsigx ',myThid)
     &                 .OR. DIAGNOSTICS_IS_ON('divsigy ',myThid)
     &                 .OR. DIAGNOSTICS_IS_ON('SIuvel  ',myThid)
     &                 .OR. DIAGNOSTICS_IS_ON('SIvvel  ',myThid)
CEAW]       
...
CEAW[ Create new statement for sigma tensor diagnostics
       diag_SIsig_isOn    = DIAGNOSTICS_IS_ON('SIsigm11',myThid)
     &                 .OR. DIAGNOSTICS_IS_ON('SIsigm22',myThid)
     &                 .OR. DIAGNOSTICS_IS_ON('SIsigm12',myThid)
CEAW]
chzh[
       diag_SIstr_isOn    = DIAGNOSTICS_IS_ON('SIstre11 ',myThid)
     &                 .OR. DIAGNOSTICS_IS_ON('SIstre22 ',myThid)
     &                 .OR. DIAGNOSTICS_IS_ON('SIstre12 ',myThid)
chzh]
       IF ( diag_SIsigma_isOn  .OR. diag_SIshear_isOn .OR.
     &      diag_SIenpi_isOn   .OR. diag_SIenpot_isOn .OR.
CEAW[ Adding my new statement to the if statement 
     &      diag_SIsig_isOn    .OR.
CEAW]
chzh[
     &      diag_SIstr_isOn    .OR.
...
C     PRESS = PRESS(n-1), n = number of sub-cycling steps
     ...
     &            + seaice_sigma12(i  ,j+1,bi,bj) )
             sigTmp = SQRT( sigm*sigm + 4.D0*sig12(i,j)*sig12(i,j) )
             recip_prs = 0.D0
             IF ( press0(i,j,bi,bj) .GT. 1.D-13 )
     &            recip_prs = 1.D0 / press0(i,j,bi,bj)
             sig1(i,j) = 0.5D0*(sigp + sigTmp)*recip_prs
             sig2(i,j) = 0.5D0*(sigp - sigTmp)*recip_prs
            ENDDO
           ENDDO
          ELSE
...
CEAW[ Filling the sigma tensor diagnostics immediately from stress routine
          CALL DIAGNOSTICS_FILL(sig1,'SIsigm11',0,1,2,bi,bj,myThid)
          CALL DIAGNOSTICS_FILL(sig2,'SIsigm22',0,1,2,bi,bj,myThid)
          CALL DIAGNOSTICS_FILL(sig12,'SIsigm12',0,1,2,bi,bj,myThid)
CEAW]
...
C     in different way to get meaningfull sig12C (=sigTmp):
             sigTmp = 2.D0 * eta(i,j,bi,bj) * 0.25D0 *
     &            (e12(i,j,bi,bj) + e12(i+1,j,bi,bj)
     &            +e12(i,j+1,bi,bj)+e12(i+1,j+1,bi,bj))
             sigTmp = SQRT( sigm*sigm + 4.D0*sigTmp*sigTmp )
             recip_prs = 0.D0
             IF ( press0(i,j,bi,bj) .GT. 1.D-13 )
     &            recip_prs = 1.D0 / press0(i,j,bi,bj)
             sig1(i,j) = 0.5D0*(sigp + sigTmp)*recip_prs
...
C     the average
         ...
         ENDIF
chzh[
         IF ( diag_SIstr_isOn  ) THEN
          DO j=1,sNy
           DO i=1,sNx
            sig1(i,j) = e11(i,j,bi,bj)
            sig2(i,j) = e22(i,j,bi,bj)
            sig12(i,j)= e12(i,j,bi,bj)
           ENDDO
          ENDDO
          CALL DIAGNOSTICS_FILL(sig1, 'SIstre11',0,1,2,bi,bj,myThid)
          CALL DIAGNOSTICS_FILL(sig2, 'SIstre22',0,1,2,bi,bj,myThid)
          CALL DIAGNOSTICS_FILL(sig12,'SIstre12',0,1,2,bi,bj,myThid)
         ENDIF
c     hzh]
...
C     therefore we need to use new arrays
     ...
     I           bi, bj, myTime, myIter, myThid )
          ...
          DO j=1,sNy+1
             DO i=1,sNx+1
                SIuvel(i,j) = uIce(i,j,bi,bj)
                SIvvel(i,j) = vIce(i,j,bi,bj)
             ENDDO
          ENDDO
CEAW[ Filling div sig diagnostics immediately after the stress div routine
C     regardless of the ALLOW_AUTODIFF option. Also outputting velocities
C     to ensure I have the same step as the other calculations.      
          CALL DIAGNOSTICS_FILL(sig1,'divsigx ',0,1,2,bi,bj,myThid)
          CALL DIAGNOSTICS_FILL(sig2,'divsigy ',0,1,2,bi,bj,myThid)
          CALL DIAGNOSTICS_FILL(SIuvel,'SIuvel  ',0,1,2,bi,bj,myThid)
          CALL DIAGNOSTICS_FILL(SIvvel,'SIvvel  ',0,1,2,bi,bj,myThid)
CEAW]
...
C     average from velocity points to pressure points
          DO j=1,sNy
           DO i=1,sNx
            sig12(i,j)= 0.5D0 * ( sig1(i,j) + sig1(i+1,j)
     &                             + sig2(i,j) + sig2(i,j+1) )
           ENDDO
          ENDDO
         ...
         IF ( diag_SIenpot_isOn ) THEN
          DO j=1,sNy
           DO i=1,sNx
            sig1(i,j) = 0.5D0 * press0(i,j,bi,bj) *
     ...
     &          )
         ...
         IF ( DIAGNOSTICS_IS_ON('SIenpa  ',myThid) ) THEN
          DO j=1,sNy+1
           DO i=1,sNx+1
            areaW = 0.5D0 * (AREA(i,j,bi,bj) + AREA(i-1,j,bi,bj))
     &           * SEAICEstressFactor
            areaS = 0.5D0 * (AREA(i,j,bi,bj) + AREA(i,j-1,bi,bj))
     &           * SEAICEstressFactor
...
C     introduce turning angle (default is zero)
     ...
     &         ( DWATN(i  ,j,bi,bj) *
     &         0.5D0*(vIce(i  ,j  ,bi,bj)-vVel(i  ,j  ,kSrf,bi,bj)
     &                  +vIce(i  ,j+1,bi,bj)-vVel(i  ,j+1,kSrf,bi,bj))
     &         + DWATN(i-1,j,bi,bj) *
     &         0.5D0*(vIce(i-1,j  ,bi,bj)-vVel(i-1,j  ,kSrf,bi,bj)
     ...
     &         + SIGN(SINWAT,  fCori(i,j,bi,bj)) * 0.5D0 *
     &         ( DWATN(i,j  ,bi,bj) *
     &         0.5D0*(uIce(i  ,j  ,bi,bj)-uVel(i  ,j  ,kSrf,bi,bj)
     &                  +uIce(i+1,j  ,bi,bj)-uVel(i+1,j  ,kSrf,bi,bj))
     &         + DWATN(i,j-1,bi,bj) *
     &         0.5D0*(uIce(i  ,j-1,bi,bj)-uVel(i  ,j-1,kSrf,bi,bj)
