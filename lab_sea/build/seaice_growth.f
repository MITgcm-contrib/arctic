...
C     Helper variables: reciprocal of some constants
      Real*8 recip_multDim
      Real*8 recip_deltaTtherm
      Real*8 recip_rhoIce
...
C     TmixLoc   :: ocean surface/mixed-layer temperature (in K)
      Real*8 TmixLoc       (1:sNx,1:sNy)

C     actual ice thickness (with upper and lower limit)
      Real*8 heffActual          (1:sNx,1:sNy)
C     actual snow thickness
      Real*8 hsnowActual         (1:sNx,1:sNy)
C     actual ice thickness (with lower limit only) Reciprocal
      Real*8 recip_heffActual    (1:sNx,1:sNy)

C     AREA_PRE  :: hold sea-ice fraction field before any seaice-thermo update
      Real*8 AREApreTH           (1:sNx,1:sNy)
      Real*8 HEFFpreTH           (1:sNx,1:sNy)
      Real*8 HSNWpreTH           (1:sNx,1:sNy)

C     wind speed
      Real*8 UG                  (1:sNx,1:sNy)

C     temporary variables available for the various computations
      Real*8 tmparr1             (1:sNx,1:sNy)

      Real*8 ticeInMult          (1:sNx,1:sNy,nITD)
      Real*8 ticeOutMult         (1:sNx,1:sNy,nITD)
      Real*8 heffActualMult      (1:sNx,1:sNy,nITD)
      Real*8 hsnowActualMult     (1:sNx,1:sNy,nITD)
      Real*8 a_QbyATMmult_cover  (1:sNx,1:sNy,nITD)
      Real*8 a_QSWbyATMmult_cover(1:sNx,1:sNy,nITD)
      Real*8 a_FWbySublimMult    (1:sNx,1:sNy,nITD)
...
C     r_QbyATM_open  :: same but for open water
      Real*8 a_QbyATM_cover      (1:sNx,1:sNy)
      Real*8 a_QbyATM_open       (1:sNx,1:sNy)
      Real*8 r_QbyATM_cover      (1:sNx,1:sNy)
      Real*8 r_QbyATM_open       (1:sNx,1:sNy)
C     a_QSWbyATM_open   - short wave heat flux over ocean in W/m^2
C     a_QSWbyATM_cover  - short wave heat flux under ice in W/m^2
      Real*8 a_QSWbyATM_open     (1:sNx,1:sNy)
      Real*8 a_QSWbyATM_cover    (1:sNx,1:sNy)
...
C             processes have been accounted for
      Real*8 a_QbyOCN            (1:sNx,1:sNy)
      Real*8 r_QbyOCN            (1:sNx,1:sNy)

C     The change of mean ice thickness due to turbulent ocean-sea ice heat fluxes
      Real*8 d_HEFFbyOCNonICE    (1:sNx,1:sNy)
...
C     the open water fraction and ice-covered fractions of the grid cell
      Real*8 d_HEFFbyATMonOCN    (1:sNx,1:sNy)
C     The change of mean ice thickness due to flooding by snow
      Real*8 d_HEFFbyFLOODING    (1:sNx,1:sNy)
...
C     water fraction and ice-covered fractions of the grid cell, respectively
      Real*8 d_HEFFbyATMonOCN_open(1:sNx,1:sNy)
      Real*8 d_HEFFbyATMonOCN_cover(1:sNx,1:sNy)

      Real*8 d_HSNWbyATMonSNW    (1:sNx,1:sNy)
      Real*8 d_HSNWbyOCNonSNW    (1:sNx,1:sNy)
      Real*8 d_HSNWbyRAIN        (1:sNx,1:sNy)

      Real*8 d_HFRWbyRAIN        (1:sNx,1:sNy)
...
C                     as EVAP (positive upward)
      Real*8 a_FWbySublim        (1:sNx,1:sNy)
      Real*8 r_FWbySublim        (1:sNx,1:sNy)
      Real*8 d_HEFFbySublim      (1:sNx,1:sNy)
      Real*8 d_HSNWbySublim      (1:sNx,1:sNy)



C ICE/SNOW stocks tendencies associated with the various melt/freeze processes
      Real*8 d_AREAbyATM   (1:sNx,1:sNy)
      Real*8 d_AREAbyOCN   (1:sNx,1:sNy)
      Real*8 d_AREAbyICE   (1:sNx,1:sNy)
C     Helper variables for diagnostics
      Real*8 DIAGarrayA    (1:sNx,1:sNy)
      Real*8 DIAGarrayB    (1:sNx,1:sNy)
      Real*8 DIAGarrayC    (1:sNx,1:sNy)
      Real*8 DIAGarrayD    (1:sNx,1:sNy)

      Real*8 SItflux     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 SIatmQnt    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 SIatmFW     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)

      Real*8 localSPfrac         (1:sNx,1:sNy)
...
C     and not in seaice_readparms and store in common block)
      ...
      ENDDO
      denominator = (2.0D0 * denominator) - 1.0D0
      recip_denominator = 1.D0 / denominator
...
C =====================
        ...
        DO j=1,sNy
         DO i=1,sNx
          a_QbyATM_cover(i,j)        = 0.0D0
          a_QbyATM_open (i,j)        = 0.0D0
          r_QbyATM_cover(i,j)        = 0.0D0
          r_QbyATM_open (i,j)        = 0.0D0

          a_QSWbyATM_open (i,j)      = 0.0D0
          a_QSWbyATM_cover(i,j)      = 0.0D0

          a_QbyOCN (i,j)             = 0.0D0
          r_QbyOCN (i,j)             = 0.0D0

          d_AREAbyATM(i,j)           = 0.0D0
          d_AREAbyICE(i,j)           = 0.0D0
          d_AREAbyOCN(i,j)           = 0.0D0

          d_HEFFbyOCNonICE(i,j)      = 0.0D0
          d_HEFFbyATMonOCN(i,j)      = 0.0D0
          d_HEFFbyFLOODING(i,j)      = 0.0D0

          d_HEFFbyATMonOCN_open(i,j) = 0.0D0
          d_HEFFbyATMonOCN_cover(i,j)= 0.0D0

          d_HSNWbyATMonSNW(i,j)      = 0.0D0
          d_HSNWbyOCNonSNW(i,j)      = 0.0D0
          d_HSNWbyRAIN(i,j)          = 0.0D0
          a_FWbySublim(i,j)          = 0.0D0
          r_FWbySublim(i,j)          = 0.0D0
          d_HEFFbySublim(i,j)        = 0.0D0
          d_HSNWbySublim(i,j)        = 0.0D0
          d_HFRWbyRAIN(i,j)          = 0.0D0
          tmparr1(i,j)               = 0.0D0
          DO IT=1,SEAICE_multDim
            ticeInMult(i,j,IT)            = 0.0D0
            ticeOutMult(i,j,IT)           = 0.0D0
            a_QbyATMmult_cover(i,j,IT)    = 0.0D0
            a_QSWbyATMmult_cover(i,j,IT)  = 0.0D0
            a_FWbySublimMult(i,j,IT)      = 0.0D0
          ENDDO
         ENDDO
        ENDDO
...
C     Compute relative wind speed over sea ice.
         DO j=1,sNy
          DO i=1,sNx
           SPEED_SQ =
     &          (uWind(i,j,bi,bj)
     &          -0.5D0*(uice(i,j,bi,bj)+uice(i+1,j,bi,bj)))**2
     &          +(vWind(i,j,bi,bj)
     &          -0.5D0*(vice(i,j,bi,bj)+vice(i,j+1,bi,bj)))**2
...
C--   assume homogeneous distribution between 0 and 2 x heffActual
         pFac = (2.0D0*IT - 1.0D0)*recip_denominator
         pFacSnow = 1.D0
...
c          IF (atemp(I,J,bi,bj) .LT. celsius2K) THEN
           IF ( a_QbyATM_cover(i,j).GE. 0.D0 ) THEN
C           add precip as snow
            d_HFRWbyRAIN(i,j)=0.D0
            d_HSNWbyRAIN(i,j)=convertPRECIP2HI*ICE2SNOW*
...
C note: in legacy, this process is done at the end
     ...
     &              +HEFF(i,j,bi,bj)*SEAICE_rhoIce)*recip_rhoConst
           tmpscal1 = MAX( 0.D0, tmpscal0 - HEFF(i,j,bi,bj))
...
C   (SEAICE_areaLossFormula.EQ.3) from predicted melt by ATM
          IF (SEAICE_areaLossFormula.EQ.1) THEN
            tmpscal3 = MIN( 0.D0 , d_HEFFbyATMonOCN_cover(i,j) )
     &        + MIN( 0.D0 , d_HEFFbyATMonOCN_open(i,j) )
     &        + MIN( 0.D0 , d_HEFFbyOCNonICE(i,j) )
...
C apply tendency
          IF ( (HEFF(i,j,bi,bj).GT.0.D0).OR.
     &        (HSNOW(i,j,bi,bj).GT.0.D0) ) THEN
           AREA(i,j,bi,bj)=MAX(0.D0,
     ...
     &       +HALF*recip_HH*MIN(0.D0,d_HEFFbyATMonOCN_open(i,j))
     &       *areaPDFfac
          d_AREAbyICE(i,j)=
     &        HALF*recip_HH*MIN(0.D0,d_HEFFbyATMonOCN_cover(i,j))
     &       *areaPDFfac
          d_AREAbyOCN(i,j)=
     &        HALF*recip_HH*MIN( 0.D0,d_HEFFbyOCNonICE(i,j) )
     &       *areaPDFfac
         ENDDO
        ENDDO
...
C ===================================================================
        ...
        IF ( useDiagnostics ) THEN
         tmpscal1=1.D0 * recip_deltaTtherm
