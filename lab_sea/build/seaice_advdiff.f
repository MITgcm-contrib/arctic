...
C     myThid    :: my Thread Id number
      Real*8 uc   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vc   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 myTime
      INTEGER myIter
      INTEGER myThid
...
C     xA,yA     :: "areas" of X and Y face of tracer cells
      ...
      Real*8 SEAICEdiffKhSItr
      Real*8 SItrExt   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 tmpscal1, tmpscal2
      Real*8 SItrPrev  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 fldNm1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      ...
      Real*8 gFld      (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 xA        (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 yA        (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 recip_heff(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEOP
...
C--   compute cell areas used by all tracers
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          xA(i,j,bi,bj) = dyG(i,j,bi,bj)*SIMaskU(i,j,bi,bj)
          yA(i,j,bi,bj) = dxG(i,j,bi,bj)*SIMaskV(i,j,bi,bj)
         ENDDO
        ENDDO
       ENDDO
      ENDDO
...
C--   Effective Thickness (Volume)
     ...
     I         bi, bj, myTime, myIter, myThid )
         IF ( SEAICEdiffKhHeff .GT. 0.D0 ) THEN
...
C--   Fractional area
     ...
     O         gFld, afx, afy,
     I         bi, bj, myTime, myIter, myThid )
         IF ( SEAICEdiffKhArea .GT. 0.D0 ) THEN
C-    Add tendency due to diffusion
          CALL SEAICE_DIFFUSION(
...
C--   Effective Snow Thickness (Volume)
     ...
     O         gFld, afx, afy,
     I         bi, bj, myTime, myIter, myThid )
         IF ( SEAICEdiffKhSnow .GT. 0.D0 ) THEN
C--   Add tendency due to diffusion
          CALL SEAICE_DIFFUSION(
...
C           treat case of potential negative value
            IF (HEFF(i,j,bi,bj).GE.siEps) THEN
             tmpscal1=MIN(0.D0,SItracer(i,j,bi,bj,iTr))
