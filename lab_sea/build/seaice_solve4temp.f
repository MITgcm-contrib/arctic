...
C----------
      ...
      Real*8 HICE_ACTUAL (1:sNx,1:sNy)
      Real*8 HSNOW_ACTUAL(1:sNx,1:sNy)
      Real*8 TSURFin     (1:sNx,1:sNy)
      Real*8 TSURFout    (1:sNx,1:sNy)
      Real*8 F_io_net    (1:sNx,1:sNy)
      Real*8 F_ia_net    (1:sNx,1:sNy)
      Real*8 F_ia        (1:sNx,1:sNy)
      Real*8 IcePenetSW  (1:sNx,1:sNy)
      Real*8 FWsublim    (1:sNx,1:sNy)
      INTEGER bi, bj
      Real*8     myTime
...
C     dFia_dTs :: derivative of surf heat flux (F_ia) w.r.t surf. temp
      Real*8 tempFrz    (1:sNx,1:sNy)
      Real*8 F_c        (1:sNx,1:sNy)
      Real*8 F_lwu      (1:sNx,1:sNy)
      Real*8 F_sens     (1:sNx,1:sNy)
      Real*8 F_lh       (1:sNx,1:sNy)
      Real*8 qhice      (1:sNx,1:sNy)
      Real*8 dqh_dTs    (1:sNx,1:sNy)
      Real*8 dFia_dTs   (1:sNx,1:sNy)
      Real*8 absorbedSW (1:sNx,1:sNy)
      Real*8 penetSWFrac
      Real*8 delTsurf

C     local copies of global variables
      Real*8 tsurfLoc   (1:sNx,1:sNy)
      Real*8 tsurfPrev  (1:sNx,1:sNy)
      Real*8 atempLoc   (1:sNx,1:sNy)
      Real*8 lwdownLoc  (1:sNx,1:sNy)
      Real*8 ALB        (1:sNx,1:sNy)
      Real*8 ALB_ICE    (1:sNx,1:sNy)
      Real*8 ALB_SNOW   (1:sNx,1:sNy)
...
C     Snow-Thickness below HCUT: linear transition to ice-albedo
      ...
      recip_HCUT = 0.D0
      IF ( HCUT.GT.0.D0 ) recip_HCUT = 1.D0 / HCUT
...
C     here in a separate array
        iceOrNot  (I,J) = HICE_ACTUAL(I,J) .GT. 0.D0
        absorbedSW(I,J) = 0.D0
        qhice    (I,J) = 0.D0
        dqh_dTs  (I,J) = 0.D0
        F_lh     (I,J) = 0.D0
...
C     If snow is present, no radiation penetrates through snow+ice
         IF (HSNOW_ACTUAL(I,J) .GT. 0.0D0) THEN
          penetSWFrac = 0.0D0
         ELSE
          penetSWFrac = XIO*EXP(-1.5D0 * HICE_ACTUAL(I,J))
         ENDIF
C     The shortwave radiative flux leaving ocean beneath ice (+=up).
         IcePenetSW(I,J) = -(1.0D0 - ALB(I,J))
     &                    *penetSWFrac * SWDOWN(I,J,bi,bj)
C     The shortwave radiative flux convergence in the seaice.
         absorbedSW(I,J) =  (1.0D0 - ALB(I,J))
     &        *(1.0D0 - penetSWFrac)* SWDOWN(I,J,bi,bj)
...
c        hice_tmp = max(HICE_ACTUAL(I,J),5.D-2)
...
C     The following form does the same, but is faster
           ...
           qhice(I,J) = bb1*mm_pi/( Ppascals -(1.0D0 - bb1)*mm_pi )
...
C     Calculate the flux terms based on the updated tsurfLoc
     ...
     &              +  F_sens(I,J) + F_lh(I,J)
C     d(F_ia)/d(Tsurf)
          dFia_dTs(I,J) = 4.0D0*D3(I,J)*t3 + D1*UG(I,J)
...
c         IF (tsurfLoc(I,J) .LT. 50.0D0 ) tsurfLoc(I,J) = TMELT
