...
C     !LOCAL VARIABLES:
      ...
      Real*8 theArea, wT_Mean, wS_Mean
      Real*8 wT_Heat, theta2PE
      Real*8 tmpVal, ddPI
      Real*8 tileArea(nSx,nSy)
      Real*8 tile_wT (nSx,nSy)
      Real*8 tile_wS (nSx,nSy)
      Real*8 tileWHeat(nSx,nSy)
      Real*8 tileTh2pe(nSx,nSy)
...
C    = Omega*Theta*DeltaPI
     ...
     &           *(theta(i,j,k,bi,bj)+theta(i,j,k-1,bi,bj))*0.5D0
