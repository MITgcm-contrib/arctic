...
C     myThid  :: my Thread Id number
      INTEGER myNr, bi, bj
      Real*8 uFld  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,myNr)
      Real*8 vFld  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,myNr)
      Real*8 wFld  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,myNr)
      Real*8 dT_lev(myNr)
      Real*8 maxCFL(3)
...
C     !LOCAL VARIABLES:
      ...
      Real*8 rTransKp1, tmpVal, recVol_dT
...
C     !LOCAL VARIABLES:
      ...
      DO bj=myByLo(myThid),myByHi(myThid)
       ...
       ENDDO
      ENDDO
      CALL GLOBAL_MAX_R8 (  uCFL,  myThid  )
      CALL GLOBAL_MAX_R8 (  vCFL,  myThid  )
      CALL GLOBAL_MAX_R8 (  wCFL,  myThid  )
