...
CEOP
      ...
      Real*8 hTmp(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
c         sIceTooHeavy = rhoConst*drF(1) / 5.D0
c         DO j=1,sNy
c          DO i=1,sNx
...
C     ... and energy diagnostic
        IF ( diag_SIenph_isOn ) THEN
         recip_deltaTtherm = 1.D0 / SEAICE_deltaTtherm
         DO bj=myByLo(myThid),myByHi(myThid)
          DO bi=myBxLo(myThid),myBxHi(myThid)
           DO j=1,sNy
            DO i=1,sNx
             hTmp(i,j,bi,bj) = 0.5D0 * SEAICE_rhoIce
     &            * 0.5D0 * (
