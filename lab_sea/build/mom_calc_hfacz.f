...
C--   Calculate open water fraction at vorticity points
      ...
      IF ( hZoption.EQ.2 ) THEN
        DO j=2-OLy,sNy+OLy
         DO i=2-OLx,sNx+OLx
c         hFacZOpen=
c    &               ( hFacW(i, j ,k,bi,bj)*rAw(i, j ,bi,bj)
c    &                +hFacW(i,j-1,k,bi,bj)*rAw(i,j-1,bi,bj) )
c    &             + ( hFacS( i ,j,k,bi,bj)*rAs( i ,j,bi,bj)
c    &                +hFacS(i-1,j,k,bi,bj)*rAs(i-1,j,bi,bj) )
c         hFacZ(i,j) = 0.25D0 * hFacZOpen*recip_rAz(i,j,bi,bj)
          hFacZOpen=
     &               ( hFacW(i, j ,k,bi,bj)
     &                +hFacW(i,j-1,k,bi,bj) )
     &             + ( hFacS( i ,j,k,bi,bj)
...
c    &                + hFacS(i-1,j,k,bi,bj)*rAs(i-1,j,bi,bj)
c    &                 )
c         hFacZ(i,j) = 0.5D0 * hFacZOpen*recip_rAz(i,j,bi,bj)
          hFacZOpen=MIN(
     &                  hFacW(i, j ,k,bi,bj)
     &                + hFacW(i,j-1,k,bi,bj)
     &                , hFacS( i ,j,k,bi,bj)
