...
C     == local variables ==
      ...
      Real*8     exf_inscal_sst, exf_inscal_sss
C     == end of interface ==
...
C     Calendar data.
      ...
      hs_period          = 0.0D0
      hs_const           = 0.0D0
      hs_exfremo_intercept = 0.0D0
      hs_exfremo_slope   = 0.0D0

      hl_startdate1      = 0
      hl_startdate2      = 0
      hl_period          = 0.0D0
      hl_const           = 0.0D0
      hl_exfremo_intercept = 0.0D0
      hl_exfremo_slope   = 0.0D0
      ...
      runoftempconst              = 0.0D0
      runoftemp_exfremo_intercept = 0.0D0
      runoftemp_exfremo_slope     = 0.0D0

      saltflxstartdate1  = 0
      saltflxstartdate2  = 0
      saltflxperiod      = 0.0D0
      saltflxconst       = 0.0D0
      saltflx_exfremo_intercept = 0.0D0
      saltflx_exfremo_slope = 0.0D0
      ...
      tidePotPeriod      = 0.0D0
      tidePotConst       = 0.0D0
      tidePot_exfremo_intercept = 0.D0
      tidePot_exfremo_slope = 0.D0
...
c     exf_inscal_sst       =  1.D0
c     exf_inscal_sss       =  1.D0
      ...
      exf_inscal_runoftemp =  1.D0
      exf_inscal_saltflx   =  1.D0
      ...
      exf_inscal_tidePot   =  1.D0
      exf_inscal_areamask  =  1.D0
...
C     Output scaling factors.
      ...
      exf_outscal_tidePot  =  1.D0
      exf_outscal_areamask =  1.D0
...
C- Note: need this, even if EXF option  is undef;
...
C        , but always call EXTERNAL_FIELDS_LOAD.
C        If  is undef, clim.relaxation could still
C        be done outside of exf.
      IF ( tauThetaClimRelax.NE.0.D0 ) THEN
...
C--   Overwrite tauSaltClimRelax but stop if already set.
      IF ( tauSaltClimRelax.NE.0.D0 ) THEN
        WRITE(msgBuf,'(2A)') 'EXF_READPARMS: ',
