...
c       hMixCriteria  = -0.8D0
c       dRhoSmall = 1.D-6
        rhoBigNb  = rhoConst*1.D10
        CALL FIND_ALPHA(
...
C-    potential density (reference level = surface level)
     ...
     &            .GT.sigmAv*hMixCriteria ) THEN
               tmpFac = 0.D0
               IF ( sigmAv.GT.0.D0 ) THEN
                 tmpFac = hMixCriteria*sigmaR(i,j,kup)/sigmaR(i,j,klw)
                 IF ( tmpFac .GT. 1.D0 ) THEN
                   tmpFac = 1.D0
     &             + ( tmpFac - 1.D0 )/( hMixCriteria - 1.D0 )
                 ENDIF
                 tmpFac = MAX( 0.D0, MIN( tmpFac, 2.D0 ) )
               ENDIF
C-    Note: sign is more related to kDir than it is to gravity orientation
     ...
     &                               - drF(k)*tmpFac*0.5D0
       ...
       IF ( hMixSmooth .GT. 0.D0 ) THEN
        tmpFac = (1.D0 - hMixSmooth ) / 4.D0
