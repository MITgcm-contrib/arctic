...
C     !ROUTINE: EXCH1_Z_R8_CUBE

C     !INTERFACE:
      SUBROUTINE EXCH1_Z_R8_CUBE(
     U                 array,
     I                 withSigns,
...
C     |   for a Zeta-located, scalar field R8 arrays.
C     *==============================================================*
...
c       WRITE(msgBuf,'(A)')'EXCH1_Z_R8_CUBE: AD mode not implemented'
c       CALL PRINT_ERROR( msgBuf, myThid )
c       STOP 'ABNORMAL END: EXCH1_Z_R8_CUBE: no AD code'
c     ENDIF
      IF ( sNx.NE.sNy .OR.
     &     nSx.NE.6 .OR. nSy.NE.1 .OR.
     &     nPx.NE.1 .OR. nPy.NE.1 ) THEN
        WRITE(msgBuf,'(2A)') 'EXCH1_Z_R8_CUBE: Wrong Tiling'
        CALL PRINT_ERROR( msgBuf, myThid )
        WRITE(msgBuf,'(2A)') 'EXCH1_Z_R8_CUBE: ',
     &   'works only with sNx=sNy & nSx=6 & nSy=nPx=nPy=1'
        CALL PRINT_ERROR( msgBuf, myThid )
        STOP 'ABNORMAL END: EXCH1_Z_R8_CUBE: Wrong Tiling'
      ENDIF
