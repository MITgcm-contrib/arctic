...
C     xPsi0,yPsi0 :: grid-point coordinate where Psi == 0
      ...
      Real*8     diagCG_resTarget
      Real*8     diagCG_pcOffDFac
      Real*8     xPsi0, yPsi0
...
C     myThid  :: my Thread Id number
      ...
      INTEGER NrMax
      Real*8     qtmp1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,NrMax,nSx,nSy)
      Real*8     qtmp2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,NrMax,nSx,nSy)
      Real*8     myTime
...
C     psiLoc  :: horizontal stream-function at special location
      ...
      Real*8  residCriter, firstResidual, minResidual, lastResidual
      Real*8  a2dMax, a2dNorm
      Real*8  rhsMax, b2dNorm, x2dNorm
      Real*8  aW2d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  aS2d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  b2d (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  x2d (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 uTrans(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vTrans(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 psiVel(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 psiLoc(2)
      INTEGER iL
...
C-    Normalise Matrice & RHS :
        ...
        IF ( normaliseMatrice ) THEN
          ...
          IF ( a2dMax .GT. 0.D0 ) a2dNorm = 1.D0/a2dMax
          b2dNorm = a2dNorm
        ENDIF
        IF ( diagNormaliseRHS ) THEN
          CALL GLOBAL_MAX_R8 (  rhsMax,  myThid  )
          IF ( rhsMax .GT. 0.D0 ) THEN
            b2dNorm = 1.D0/rhsMax
