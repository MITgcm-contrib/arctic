...
C     i, j   :: Loop counters
      ...
      INTEGER  i,  j
C     pseudo velocities
      Real*8 uPseudo(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 vPseudo(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 uC, vC, uNorm, tmpVal
CEOP
        ...
        IF ( .NOT.skipCalcAngleC ) THEN
         DO j = 1-OLy,sNy+OLy-1
          DO i = 1-OLx,sNx+OLx-1
           ...
           IF ( uNorm .NE. 0.D0 ) uNorm = 1.D0/uNorm
...
c         tmpVal = dxG(i,j,bi,bj) * COS( deg2rad*
          tmpVal = rAs(i,j,bi,bj) * COS( deg2rad*
     &             ( yG(i,j,bi,bj) + yG(i+1,j,bi,bj) )*halfRL )
          IF ( tmpVal.GT.0. ) THEN
            v2zonDir(i,j,bi,bj) = -rSphere
     &          *( SIN( yG(i+1,j,bi,bj)*deg2rad )
     &           - SIN( yG(i,j,bi,bj)*deg2rad )
     &           )* dyC(i,j,bi,bj)/tmpVal
c    &           )/tmpVal
          ELSE
