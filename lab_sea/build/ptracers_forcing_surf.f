...
C  myThid               :: thread number
      Real*8 relaxForcingS(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C  ks                   :: surface level index
      INTEGER i, j
      INTEGER iTrc, ks
      Real*8 add2EmP(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 epsil, cutoff, tmpVar
CEOP
...
C       current limit is set to 0.1 CFL
        epsil = 1.D-10
        cutoff = 0.1D0 *drF(ks)/PTRACERS_dTLev(ks)
