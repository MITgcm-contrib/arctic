...
C     i, j, it2d :: Loop counters ( it2d counts CG iterations )
      INTEGER bi, bj
      INTEGER i, j, it2d
      Real*8    err,     errTile(nSx,nSy)
      Real*8    eta_qrN, eta_qrNtile(nSx,nSy)
      Real*8    eta_qrNM1
      Real*8    cgBeta
      Real*8    alpha,  alphaTile(nSx,nSy)
      Real*8    sumRHS, sumRHStile(nSx,nSy)
      Real*8    pW_tmp, pS_tmp
...
CEOP
      ...
      Real*8  q2d(0:sNx+1,0:sNy+1,nSx,nSy)
      Real*8  r2d(0:sNx+1,0:sNy+1,nSx,nSy)
        ...
        DO j=1,sNy+1
         DO i=1,sNx+1
          IF ( aC2d(i,j,bi,bj) .EQ. 0. ) THEN
           pC(i,j,bi,bj) = 1.D0
          ELSE
           pC(i,j,bi,bj) = 1.D0 / aC2d(i,j,bi,bj)
          ENDIF
