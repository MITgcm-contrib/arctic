...
C     !LOCAL VARIABLES:
      ...
      INTEGER i,j,k
      INTEGER ks, kp1
      Real*8 numPnts,theVol,tmpVal, mskp1, msk_1
      Real*8 abFac1, abFac2, R_drK, cosLat
      Real*8 theMax,theMean,theVolMean,potEnMean
      Real*8 totAMu, totAMs
      Real*8 tileMean(nSx,nSy)
      Real*8 tileVlAv(nSx,nSy)
      Real*8 tilePEav(nSx,nSy)
      Real*8 tileVol (nSx,nSy)
      Real*8 tileAMu (nSx,nSy)
      Real*8 tileAMs (nSx,nSy)
      Real*8 tmpFld(1:sNx,1:sNy)
      Real*8 cos2LatG(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C- Potential Energy (external mode):
         DO j=1,sNy
          DO i=1,sNx
           tmpVal = 0.5D0*Bo_surf(i,j,bi,bj)
...
c    &        + 0.5D0*Bo_surf(i,j,bi,bj)*tmpVal*tmpVal
...
C- end bi,bj loops
       ENDDO
      ENDDO
      CALL GLOBAL_SUM_R8 ( numPnts, myThid )
...
C-    Calculate contribution from zonal velocity
         abFac1 = 0.D0
         abFac2 = 0.D0
          IF ( myIter.GE.1 ) abFac1 = -( 0.5D0 + abEps )
C-    contribution from uVel component: 1rst integrate vertically
     ...
     &             + R_drK*tmpVal*hFacW(i,j,k,bi,bj)
           ENDDO
          ENDDO
         ENDDO
     ...
     &                   *rhoFacC(k)*drF(k)
          DO j=1,sNy
           DO i=1,sNx
            ...
            tmpFld(i,j) = tmpFld(i,j)
     &             + R_drK*tmpVal*hFacS(i,j,k,bi,bj)
           ENDDO
          ENDDO
         ENDDO
C-    and then integrate horizontally over this tile
