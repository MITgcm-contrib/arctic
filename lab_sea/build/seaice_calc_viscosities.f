...
C     factor k to compute tensile strength from k*press0
      Real*8 tnsFac(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     bulk viscosity
...
C     em            - e11-e22 (abbreviation)
      INTEGER i, j, bi, bj
      Real*8 recip_e2, deltaCsq, deltaMinSq, tmp, ep, em, smallNbr
      Real*8 recip_efr4, recip_efr2, oneThird, oneNinth, smallNbrSq
      Real*8 e12Csq     (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 deltaCreg  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 recip_shear(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 shearDefSq, shearDef
      Real*8 sumNorm, maskZ
CEOP
...
C--   basic constants
      ...
      recip_e2=0.D0
      IF ( SEAICE_eccen .NE. 0.D0 ) recip_e2=ONE/(SEAICE_eccen**2)
      recip_efr2=0.D0
      recip_efr4=0.D0
      IF( SEAICE_eccfr .NE. 0.D0) THEN
     ...
     &          + rAz(i+1,j+1,bi,bj)*e12(i+1,j+1,bi,bj)**2 )
        ...
        DO j=1-OLy+1,sNy+OLy-1
         DO i=1-OLx+1,sNx+OLx-1
          ...
          shearDefSq = em*em + 4.D0 * e12Csq(i,j)
          shearDef = SQRT(shearDefSq)
          recip_shear(i,j) = 1.D0 / MAX( ShearDef, smallNbr )
         ENDDO
        ENDDO
...
C     "replacement pressure"
     ...
     &          * SEAICEpressReplFac/( 1.D0 + tnsFac(i,j,bi,bj) )
     &          ) * ( 1.D0 - tnsFac(i,j,bi,bj) )
CML          press(i,j,bi,bj) = press0(i,j,bi,bj) *
CML     &         ( 1.D0 + SEAICEpressReplFac
CML     &                  * ( deltaC(i,j,bi,bj)/deltaCreg(i,j) - 1.D0 )
CML     &         ) * ( 1.D0 - tnsFac(i,j,bi,bj) )
          ENDDO
         ENDDO
