...
C     SEAICE_timeAve :: cumulated time [s]
     ...
     &        UICEtave, VICEtave, HEFFtave, AREAtave
      ...
      Real*8 SEAICE_timeAve(nSx,nSy)
      Real*8 FUtave    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 FVtave    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 EmPmRtave (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 QNETtave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 QSWtave   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 UICEtave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 VICEtave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 HEFFtave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 AREAtave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
       ...
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          HEFF(i,j,bi,bj)=0.D0
          AREA(i,j,bi,bj)=0.D0
          HSNOW(i,j,bi,bj)   = 0.D0
          UICE(i,j,bi,bj)=0.D0
          VICE(i,j,bi,bj)=0.D0
C
          uIceNm1(i,j,bi,bj) = 0.D0
          vIceNm1(i,j,bi,bj) = 0.D0
          DWATN  (i,j,bi,bj) = 0.D0
          stressDivergenceX(i,j,bi,bj) = 0.D0
          stressDivergenceY(i,j,bi,bj) = 0.D0
          seaice_sigma1 (i,j,bi,bj) = 0.D0
          seaice_sigma2 (i,j,bi,bj) = 0.D0
          seaice_sigma12(i,j,bi,bj) = 0.D0
          e11    (i,j,bi,bj) = 0.D0
          e22    (i,j,bi,bj) = 0.D0
          e12    (i,j,bi,bj) = 0.D0
          deltaC (i,j,bi,bj) = 0.D0
          PRESS  (i,j,bi,bj) = 0.D0
          ETA    (i,j,bi,bj) = 0.D0
          etaZ   (i,j,bi,bj) = 0.D0
          ZETA   (i,j,bi,bj) = 0.D0
          FORCEX (i,j,bi,bj) = 0.D0
          FORCEY (i,j,bi,bj) = 0.D0
          tensileStrFac(i,j,bi,bj) = 0.D0
          PRESS0(i,j,bi,bj)  = 0.D0
          FORCEX0(i,j,bi,bj) = 0.D0
          FORCEY0(i,j,bi,bj) = 0.D0
          SEAICE_zMax(i,j,bi,bj) = 0.D0
          SEAICE_zMin(i,j,bi,bj) = 0.D0
          seaiceMassC(i,j,bi,bj)=0.D0
          seaiceMassU(i,j,bi,bj)=0.D0
          seaiceMassV(i,j,bi,bj)=0.D0
          uice_fd(i,j,bi,bj) = 0.D0
          vice_fd(i,j,bi,bj) = 0.D0
          CbotC(i,j,bi,bj)   = 0.D0
          DO iTr = 1, SItrMaxNum
           SItracer(i,j,bi,bj,iTr) = 0.D0
           SItrBucket(i,j,bi,bj,iTr) = 0.D0
c "ice concentration" tracer that should remain .EQ.1.
           if (SItrName(iTr).EQ.'one') SItracer(i,j,bi,bj,iTr)=1.D0
          ENDDO
          DO jTh = 1, 5
           SItrHEFF (i,j,bi,bj,jTh) = 0.D0
          ENDDO
          DO jTh = 1, 3
           SItrAREA (i,j,bi,bj,jTh) = 0.D0
          ENDDO
          DO k=1,nITD
            TICES(i,j,k,bi,bj)=0.D0
          ENDDO
          saltWtrIce(i,j,bi,bj) = 0.D0
          frWtrIce(i,j,bi,bj)   = 0.D0
         ENDDO
        ENDDO
       ENDDO
      ENDDO
       ...
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy+1,sNy+OLy
         DO i=1-OLx+1,sNx+OLx
          seaiceMaskU(i,j,bi,bj)=   0.0D0
          seaiceMaskV(i,j,bi,bj)=   0.0D0
          mask_uice=HEFFM(i,j,bi,bj)+HEFFM(i-1,j  ,bi,bj)
          IF(mask_uice.GT.1.5D0) seaiceMaskU(i,j,bi,bj)=1.0D0
          mask_uice=HEFFM(i,j,bi,bj)+HEFFM(i  ,j-1,bi,bj)
          IF(mask_uice.GT.1.5D0) seaiceMaskV(i,j,bi,bj)=1.0D0
         ENDDO
        ENDDO
        ...
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          DO k=1,nITD
           TICES(i,j,k,bi,bj)=273.0D0
          ENDDO
          seaiceMassC(i,j,bi,bj)=1000.0D0
          seaiceMassU(i,j,bi,bj)=1000.0D0
          seaiceMassV(i,j,bi,bj)=1000.0D0
         ENDDO
        ENDDO
...
C--   Read initial sea-ice area from file if available.
       ...
       DO bj=myByLo(myThid),myByHi(myThid)
        DO bi=myBxLo(myThid),myBxHi(myThid)
         DO j=1-OLy,sNy+OLy
          DO i=1-OLx,sNx+OLx
           HSNOW(i,j,bi,bj) = 0.2D0 * AREA(i,j,bi,bj)
          ENDDO
         ENDDO
        ENDDO
       ENDDO
...
C--   Read initial sea ice age from file if available.
     ...
     &   SItracer(1-OLx,1-OLy,1,1,iTr), 0, myThid )
        CALL EXCH_XY_RL ( SItracer(1-OLx,1-OLy,1,1,iTr), myThid )
        ENDIF
       ENDDO
...
C---  Complete initialization
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          ZETA(i,j,bi,bj)        = HEFF(i,j,bi,bj)*(1.0D11)
...
C     on depth
      IF ( SEAICE_tensilFac .NE. 0.D0 ) THEN
       recip_tensilDepth = 0.D0
       IF ( SEAICE_tensilDepth .GT. 0.D0 )
     &      recip_tensilDepth = 1.D0 / SEAICE_tensilDepth
       DO bj=myByLo(myThid),myByHi(myThid)
        DO bi=myBxLo(myThid),myBxHi(myThid)
         DO j=1-OLy,sNy+OLy
          DO i=1-OLx,sNx+OLx
