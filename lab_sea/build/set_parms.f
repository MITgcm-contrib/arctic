...
C--   Advection and Forcing for Temp and salt  on/off flags
      ...
      saltVertDiff4 = .FALSE.
      DO k=1,Nr
        tempVertDiff4 = tempVertDiff4 .OR. ( diffKr4T(k).GT.0.D0 )
        saltVertDiff4 = saltVertDiff4 .OR. ( diffKr4S(k).GT.0.D0 )
      ENDDO
     ...
     &             .AND. ( tauThetaClimRelax.GT.0.D0 )
      doSaltClimRelax  = ( saltForcing .OR.( useOffLine.AND.useKPP ) )
     &             .AND. ( tauSaltClimRelax .GT.0.D0 )
...
C     note: done later (once domain size is known) if using CartesianGrid
      IF ( latBandClimRelax .EQ. UNSET_RL) THEN
        IF ( usingSphericalPolarGrid ) latBandClimRelax= 180.D0
        IF ( usingCurvilinearGrid )    latBandClimRelax= 180.D0
      ENDIF
...
C     several places, test for last iteration with time==endTime :
      ...
      IF ( endTime.NE.tmpVar ) THEN
       IF ( ABS(endTime-tmpVar).GT.deltaTClock*1.D-6 ) THEN
        WRITE(msgBuf,'(A,A)') '** WARNING ** SET_PARMS: ',
