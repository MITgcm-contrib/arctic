...
C     !LOCAL VARIABLES:
      ...
      Real*8 plumefrac(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 plumetend(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      integer two2
      parameter(two2 = 2)
      Real*8 plumekb(two2), SPdepth(two2)
...
C Penetrating saltplume fraction:cumulativeSP(k+1)-cumulativeSP(k)
     ...
     &        *recip_drF(k)*mass2rUnit*recip_hFacC(i,j,k,bi,bj)
        ELSE
         plumefrac(I,J) = 0.D0
         plumetend(I,J) = 0.D0
        ENDIF
