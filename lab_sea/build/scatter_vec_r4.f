...
CBOP
C !ROUTINE: SCATTER_VEC_R4
C !INTERFACE:
      SUBROUTINE SCATTER_VEC_R4(
     I                  gloBuff,
     O                  myField,
     I                  length,
     I                  myThid )
...
C gloBuff   ( Real*4 ) :: full-domain IO-buffer array      (Input)
C myField   ( Real*4 ) :: local (i.e. my Proc.) 1D array  (Output)
C length   (integer):: size of local 1D array
C myThid   (integer):: my Thread Id number

      INTEGER length
      Real*4     gloBuff(length*nPx*nPy)
      Real*4     myField(length)
      INTEGER myThid
CEOP
