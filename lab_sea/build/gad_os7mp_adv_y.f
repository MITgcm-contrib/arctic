...
C Do not use with OBCS!
     ...
     O           vT,
     I           myThid )
C     /==========================================================C     | SUBROUTINE GAD_OS7MP_ADV_Y                               |
...
C        MP limiter
         ...
         C = d2
         D = d2p1
         dp1h = max(min(min(A,B),min(C,D)),0.D0)
     &        + min(max(max(A,B),max(C,D)),0.D0)
         A = 4.D0*d2m1 - d2
         B = 4.D0*d2 - d2m1
         C = d2m1
         D = d2
         dm1h = max(min(min(A,B),min(C,D)),0.D0)
     &        + min(max(max(A,B),max(C,D)),0.D0)
...
C--
     ...
     &                 min(min(0.D0,2.D0*rp1h_cfl),PhiLC) )
         PhiMax = min( max(2.D0/(1.D0-cfl),PhiMD),
     &                 max(max(0.D0,2.D0*rp1h_cfl),PhiLC) )
