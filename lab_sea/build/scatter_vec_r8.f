...
C gloBuff   ( Real*8 ) :: full-domain IO-buffer array      (Input)
C myField   ( Real*8 ) :: local (i.e. my Proc.) 1D array  (Output)
C length   (integer):: size of local 1D array
C myThid   (integer):: my Thread Id number

      INTEGER length
      Real*8     gloBuff(length*nPx*nPy)
      Real*8     myField(length)
      INTEGER myThid
CEOP
