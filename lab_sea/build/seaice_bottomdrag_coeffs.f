...
C     myThid    :: my Thread Id. number
      Real*8 uIceLoc   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vIceLoc   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 HEFFMLoc  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 HEFFLoc   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 AREALoc   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 CbotLoc   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      INTEGER iStep
      Real*8     myTime
      INTEGER myIter
      INTEGER myThid
...
C     i,j,bi,bj,ksrf :: loop indices
      ...
      Real*8     tmp, hActual, hCrit, recip_k1, u0sq, fac, rFac
CEOP

      IF (SEAICEbasalDragK2.GT.0.D0) THEN
...
C     some abbreviations
      ...
      recip_k1 = 0.D0
      IF ( SEAICEbasalDragK1 .GT. 0.D0 )
     &     recip_k1 = 1.D0/SEAICEbasalDragK1
C     fac scales the soft maximum for more accuracy
      ...
      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          CbotLoc(i,j,bi,bj) = 0.D0
          tmpFld (i,j)       = 0.D0
         ENDDO
        ENDDO
        DO j=1-OLy,sNy+OLy-1
         DO i=1-OLx,sNx+OLx-1
          IF ( AREALoc(i,j,bi,bj) .GT. 0.01D0 ) THEN
           tmp = 0.25D0*(
...
CML     &            tmpFld(i,j) * MAX( hActual - hCrit, 0.D0)
            CbotLoc(i,j,bi,bj) = CbotLoc(i,j,bi,bj)
     &           + tmpFld(i,j)
     &           * LOG(EXP( fac*(hActual - hCrit) ) + 1.D0)*rFac
