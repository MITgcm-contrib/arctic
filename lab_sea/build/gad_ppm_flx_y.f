...
C     ================================================================
          ...
          do  iy = 1-OLy+3, sNy+OLy-2
              ...
              if (vvel(ix,iy) .eq. 0.D0) then

                  flux(ix,iy)    = 0.D0

              else

              if (vvel(ix,iy) .gt. 0.D0) then

C     ==================== integrate PPM profile over upwind cell IY-1
              if ( calc_CFL ) then
...
C     ==================== integrate PPM profile over upwind cell IY+0
              if ( calc_CFL ) then
              vCFL = vvel(ix,iy) * delT
...
c             intF = 0.5D0 * intF / vCFL
C-    to avoid potential underflow:
              intF = 0.5D0 * intF / sign(max(abs(vCFL),1.d-20),vCFL)
...
c     end subroutine GAD_PPM_FLX_Y
      end
