...
C--  o KPP_CALC_DUMMY   - Dummy routine for TAMC
...
c     kmtj   (nx,ny)       - maximum number of wet levels in each column
      ...
      Real*8 temp1   ( 1-OLx:sNx+OLx, 1-OLy:sNy+OLy, Nr          )
      Real*8 temp2   ( 1-OLx:sNx+OLx, 1-OLy:sNy+OLy, Nr          )
...
CMLCADJ STORE ustar = comlev1_kpp, key = ikey
CML#endif
CMLcph)
...
C     the bottom of the mixing layer.
     ...
     I      myTime, myIter, myThid )
       DO j=1-OLy,sNy+OLy
        DO i=1-OLx,sNx+OLx
         KPPplumefrac(i,j,bi,bj) = 1.D0 - worka(i,j)
        ENDDO
       ENDDO
      ENDIF
