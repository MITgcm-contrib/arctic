...
C     myThid :: My Thread Id. number
      ...
      Real*8 e11Loc (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 e22Loc (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 e12Loc (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      INTEGER iStep
      Real*8     myTime
      INTEGER myIter
      INTEGER myThid
...
C     hFacU, hFacV :: determine the no-slip boundary condition
      Real*8 hFacU, hFacV, noSlipFac
      Real*8 third
      PARAMETER ( third = 0.333333333333333333333333333D0 )
C     auxillary variables that help writing code that
C     vectorizes even after TAFization
      Real*8 dudx (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 dvdy (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 dudy (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 dvdx (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 uave (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 vave (1-OLx:sNx+OLx,1-OLy:sNy+OLy)

      noSlipFac = 0.D0
      IF ( SEAICE_no_slip ) noSlipFac = 1.D0
C     in order repoduce results before fixing a bug in r1.20 comment out
C     the following line
CML      IF ( SEAICE_no_slip ) noSlipFac = 2.D0
C
...
C     for vectorization
        DO j=1-OLy,sNy+OLy-1
         DO i=1-OLx,sNx+OLx-1
          dudx(i,j) = recip_dxF(i,j,bi,bj) *
     &         (uFld(i+1,j,bi,bj)-uFld(i,j,bi,bj))
          uave(i,j) = 0.5D0 * (uFld(i,j,bi,bj)+uFld(i+1,j,bi,bj))
         ENDDO
        ENDDO
        DO j=1-OLy,sNy+OLy-1
         DO i=1-OLx,sNx+OLx-1
          dvdy(i,j) = recip_dyF(i,j,bi,bj) *
     &         (vFld(i,j+1,bi,bj)-vFld(i,j,bi,bj))
          vave(i,j) = 0.5D0 * (vFld(i,j,bi,bj)+vFld(i,j+1,bi,bj))
         ENDDO
        ENDDO
...
C     for vectorization
     ...
     &         * recip_dyU(i,j,bi,bj)
          uave(i,j) = 0.5D0 * (uFld(i,j,bi,bj)+uFld(i  ,j-1,bi,bj))
         ENDDO
        ENDDO
        DO j=1-OLy+1,sNy+OLy
         DO i=1-OLx+1,sNx+OLx
     ...
     &         * recip_dxV(i,j,bi,bj)
          vave(i,j) = 0.5D0 * (vFld(i,j,bi,bj)+vFld(i-1,j  ,bi,bj))
         ENDDO
        ENDDO
C     evaluate strain rates at Z-points
     ...
     &         + noSlipFac * (
     &           2.0D0 * uave(i,j) * recip_dyU(i,j,bi,bj) * hFacU
     &         + 2.0D0 * vave(i,j) * recip_dxV(i,j,bi,bj) * hFacV
     &         )
...
C     The masking is ugly, but hopefully effective.
           e12Loc(i,j,bi,bj) = e12Loc(i,j,bi,bj) + 0.5D0 * (
     &            recip_dyU(i,j,bi,bj) * ( 6.0D0 * uave(i,j)
     &          - uFld(i,j-2,bi,bj)*SIMaskU(i,j-1,bi,bj)
     &          - uFld(i,j+1,bi,bj)*SIMaskU(i,j  ,bi,bj) ) * hFacU
     &          + recip_dxV(i,j,bi,bj) * ( 6.0D0 * vave(i,j)
