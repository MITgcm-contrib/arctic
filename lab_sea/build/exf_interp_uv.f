...
C  arrUout     ( Real*8 )   :: 1rst component (U) output array
C  arrVout     ( Real*8 )   :: 2nd  component (V) output array
...
C  myThid      (integer) :: My Thread Id number
      ...
      Real*8           arrUout(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8           arrVout(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8           xG_in  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8           yG     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8           lon_0, lon_inc
c     Real*8           lat_0, lat_inc(nyIn-1)
      Real*8           lat_0, lat_inc(*)
...
C     arrVin      :: 2nd  component input field array (loaded from file)
      Real*8      arrUin( -1:nxIn+2, -1:nyIn+2 )
      Real*8      arrVin( -1:nxIn+2, -1:nyIn+2 )
      Real*8      csLon(-1:nxIn+2), snLon(-1:nxIn+2)
      Real*8      x_in (-1:nxIn+2), y_in (-1:nyIn+2)
      ...
      Real*8      threeSixtyRS
      Real*8      yPole, symSign, poleU, poleV
      Real*8      csdLon, sndLon, pSign
      Real*8      edgeFac, poleFac
      LOGICAL  calcLonCS
...
C       1 + truncated log2(# interval -1); add epsil=1.e-3 for safey
        tmpVar = nyIn + 1.D-3
        nLoop = 1 + INT( LOG(tmpVar)/LOG(2.D0) )
        DO l=1,nLoop
         DO j=1,sNy
          DO i=1,sNx
