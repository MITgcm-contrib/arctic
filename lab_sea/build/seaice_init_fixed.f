...
C-    loops on tile indices bi,bj:
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          ...
          SIMaskU(i,j,bi,bj) = 0.D0
          SIMaskV(i,j,bi,bj) = 0.D0
         ENDDO
        ENDDO
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
...
C     coefficients for metric terms
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          k1AtC(i,j,bi,bj) = 0.0D0
          k1AtZ(i,j,bi,bj) = 0.0D0
          k2AtC(i,j,bi,bj) = 0.0D0
          k2AtZ(i,j,bi,bj) = 0.0D0
         ENDDO
        ENDDO
...
C     coordinates when the grid is rotated (phi/theta/psiEuler .NE. 0)
         DO j=1-OLy,sNy+OLy
          DO i=1-OLx,sNx+OLx
           k2AtC(i,j,bi,bj) = - tanPhiAtU(i,j,bi,bj)*recip_rSphere
           k2AtZ(i,j,bi,bj) = - tanPhiAtV(i,j,bi,bj)*recip_rSphere
          ENDDO
         ENDDO
...
C     approximation
         DO j=1-OLy,sNy+OLy
          DO i=1-OLx,sNx+OLx-1
           k1AtC(i,j,bi,bj) = recip_dyF(i,j,bi,bj)
     ...
     &          * recip_dxF(i,j,bi,bj)
         ...
         DO j=1-OLy,sNy+OLy
          DO i=1-OLx+1,sNx+OLx
           k1AtZ(i,j,bi,bj) = recip_dyU(i,j,bi,bj)
     &          * ( dyC(i,j,bi,bj) - dyC(i-1,j,bi,bj) )
     &          * recip_dxV(i,j,bi,bj)
          ENDDO
         ENDDO
         DO j=1-OLy,sNy+OLy-1
          DO i=1-OLx,sNx+OLx
           k2AtC(i,j,bi,bj) = recip_dxF(i,j,bi,bj)
     ...
     &          * recip_dyF(i,j,bi,bj)
         ...
         DO j=1-OLy+1,sNy+OLy
          DO i=1-OLx,sNx+OLx
           k2AtZ(i,j,bi,bj) = recip_dxV(i,j,bi,bj)
     &          * ( dxC(i,j,bi,bj) - dxC(i,j-1,bi,bj) )
     &          * recip_dyU(i,j,bi,bj)
          ENDDO
         ENDDO
        ENDIF
