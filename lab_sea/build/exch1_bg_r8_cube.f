...
C     !ROUTINE: EXCH1_BG_R8_CUBE

C     !INTERFACE:
      SUBROUTINE EXCH1_BG_R8_CUBE(
     U                 uField, vField,
     I                 withSigns,
...
C     !DESCRIPTION:
C     *==========================================================*
C     | SUBROUTINE EXCH1_BG_R8_CUBE
C     | o Forward-mode edge exchanges for R8 vector on CS config:
C     |   Fill overlap region through tile exchanges,
C     |   according to CS topology,
C     |   for a 2-Components B-Grid vector field R8 arrays.
C     *==========================================================*
...
C     myThid      :: my Thread Id number
      ...
      Real*8 uField( 1-myOLw:sNx+myOLe, 1-myOLs:sNy+myOLn,
     &            myNz, nSx, nSy )
      Real*8 vField( 1-myOLw:sNx+myOLe, 1-myOLs:sNy+myOLn,
     &            myNz, nSx, nSy )
      LOGICAL withSigns
      INTEGER exchWidthX
      INTEGER exchWidthY
...
c       WRITE(msgBuf,'(A)')'EXCH1_BG_R8_CUBE: AD mode not implemented'
c       CALL PRINT_ERROR( msgBuf, myThid )
c       STOP 'ABNORMAL END: EXCH1_BG_R8_CUBE: no AD code'
c     ENDIF
      IF ( sNx.NE.sNy .OR.
     &     nSx.NE.6 .OR. nSy.NE.1 .OR.
     &     nPx.NE.1 .OR. nPy.NE.1 ) THEN
        WRITE(msgBuf,'(2A)') 'EXCH1_BG_R8_CUBE: Wrong Tiling'
        CALL PRINT_ERROR( msgBuf, myThid )
        WRITE(msgBuf,'(2A)') 'EXCH1_BG_R8_CUBE: ',
     &   'works only with sNx=sNy & nSx=6 & nSy=nPx=nPy=1'
        CALL PRINT_ERROR( msgBuf, myThid )
        STOP 'ABNORMAL END: EXCH1_BG_R8_CUBE: Wrong Tiling'
      ENDIF
