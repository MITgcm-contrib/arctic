...
C     ================ mask local stencil: expand from centre outwards
     ...
     &          + mloc(+0)*(fbar(ix+0)-fbar(ix-1))
              ...
              ftmp = 2.D0 * floc(-1) - floc(+0)
              floc(-2) = ftmp
     &          + mloc(-2)*(fbar(ix-2)-ftmp)
              ftmp = 2.D0 * floc(-2) - floc(-1)
              floc(-3) = ftmp
...
C     ================ centred, 5th-order interpolation for edge value
              edge(1,ix) =
     &    +( 1.D0/60.D0)*(floc(-3)+floc(+2))
     &    -( 8.D0/60.D0)*(floc(-2)+floc(+1))
     &    +(37.D0/60.D0)*(floc(-1)+floc(+0))

              edge(2,ix) = (
     &    -( 1.D0/90.D0)*(floc(-3)-floc(+2))
     &    +( 5.D0/36.D0)*(floc(-2)-floc(+1))
     &    -(49.D0/36.D0)*(floc(-1)-floc(+0))
