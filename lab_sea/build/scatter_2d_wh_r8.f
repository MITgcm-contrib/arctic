...
CBOP
C !ROUTINE: SCATTER_2D_WH_R8
C !INTERFACE:
      SUBROUTINE SCATTER_2D_WH_R8(
     I                  gloBuff,
     O                  procBuff,
     I                  myThid )
C !DESCRIPTION:
...
C gloBuff   ( Real*8 ) :: full-domain 2D IO-buffer array             (Input)
C procBuff  ( Real*8 ) :: proc-domain 2D IO-buffer array             (Input)
...
C     gloNyWh :: global sum of sNyWh
      ...
      Real*8     gloBuff(sNxWh,gloNyWh)
      Real*8     procBuff(sNxWh,procNyWh)
      INTEGER myThid
CEOP
