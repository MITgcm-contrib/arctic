...
C     === Local variables ===
      ...
      Real*8 tmpscal1,tmpscal2,tmpscal3,tmpscal4

      Real*8 taux_onIce_cntr, tauy_onIce_cntr, uvel_cntr, vvel_cntr
      Real*8 mIceCor, rhs_x, rhs_y, rhs_n, rhs_a, sol_n, sol_a

      Real*8 uice_cntr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vice_cntr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
       ...
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          ...
          uice_cntr(i,j,bi,bj)=0.D0
          Vice_cntr(i,j,bi,bj)=0.D0
         ENDDO
        ENDDO
       ENDDO
      ENDDO
...
C mass of ice per unit area (kg/m2) times coriolis f
          mIceCor=SEAICE_rhoIce*HEFF(i,j,bi,bj)*fCori(i,j,bi,bj)
...
C ===============
          IF ( YC(i,j,bi,bj) .LT. ZERO ) THEN
           tmpscal1 = 1.D0 /rhoConst/SEAICE_waterDrag_south
          ELSE
           tmpscal1 = 1.D0 /rhoConst/SEAICE_waterDrag
          ENDIF
C polynomial coefficients
          tmpscal2= tmpscal1*tmpscal1*mIceCor*mIceCor
          tmpscal3= tmpscal1*tmpscal1*rhs_n*rhs_n
C discriminant
          tmpscal4=tmpscal2*tmpscal2+4.D0*tmpscal3
